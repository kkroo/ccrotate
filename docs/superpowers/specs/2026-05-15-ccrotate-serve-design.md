...
4. **Follow-ups discovered during rollout** (2026-05-15) — **ALL FIXED in image `944a1f70`**
   - ✅ **Classifier regex gap** — FIXED. `classifyQuotaError` now treats any `rate_limit_error` as structural and reads reset epoch from `anthropic-ratelimit-tokens-reset` / `retry-after` headers when present. Two new tests cover the production message format and header-based reset parsing. Test count: 122 (was 121).
   - ✅ **LiteLLM fallback chain expanded** — FIXED. Added `claude-sonnet-4-6 → claude-haiku-4-5-20251001`, `claude-opus-4-7 → claude-sonnet-4-6 → claude-haiku-4-5-20251001`, and `text-embedding-3-small → text-embedding-3-large` fallback rules. Verified end-to-end: a `claude-sonnet-4-6` request returned content from `claude-haiku-4-5-20251001` (fallback fired correctly).
   - ✅ **Per-agent bank re-consolidation** — proven for forward-going docs. Fresh POST to bank `386c81e8` returned `success:true` with `usage: {input_tokens:3351, output_tokens:481}` (real LLM activity). Bank went from `docs=7 nodes=0 links=0` to `docs=8 nodes=2 links=4`. The 87 legacy docs whose retain ops completed-empty before the cutover still need a separate re-POST workflow (their `documents` rows have 0 `chunks` and 0 `memory_units` — re-running consolidation alone is a no-op because there's nothing to consolidate; the retain step itself must re-run).
   - ✅ **current.json coordination** — documented as architectural note. Sidecar is the only cluster writer for now; v2 hardening can either teach the bot container to update `current.json` on snap operations, or have the sidecar fall back to scanning profiles for the freshest credential. No code change needed yet.
   - ✅ **Codex pool re-probe confirmed Gate 1 FAIL is correct**: codex `id_token` (chatgpt-account OAuth) against `api.openai.com` returns HTTP 401 `invalid_claims` — the token audience is chatgpt.com's backend, not the public API. Codex via CLI (different protocol) remains v2 work.

5. **Wiring proven end-to-end** (after fixes)
   - Direct probe from `hindsight` namespace through LiteLLM → ccrotate-serve → api.anthropic.com returned a `claude-haiku-4-5-20251001` chat-completion ("PONG").
   - Sonnet-shape request fell through to haiku via the new fallback chain ("SONNET-PONG" returned with `model: claude-haiku-4-5-20251001`).
   - Fresh memory POST against a per-agent bank produced `memory_units` and `chunks` (nodes+links).
   - Hindsight worker actively processing 7,453 `agent-memories` unconsolidated docs through the new path.

6. **Outstanding ops work for completeness** (not blocking)
   - **Legacy 87-doc re-POST**: the per-agent banks have documents whose retain step pre-dates the working LLM. Either re-POST each document via `/v1/<schema>/banks/<bid>/memories` (which will create memory_units this time) or run a forensic SQL+API job that walks `documents` table, fetches `original_text`, and re-posts. Out of scope for ccrotate-serve itself; tracked here as the last open thread from the original handoff.
