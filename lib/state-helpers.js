...
export function readExhaustion(entry) {
  ...
  // model-scoped `exhausted` map while preserving the markAccountExhausted-
  // shaped `response` + `rateLimits.reset5h/7d`. Most commonly this is
  // `mergeTierCacheEntry` in ccrotate.js spreading `nextEntry` (which
  // `createTierCacheEntry` builds without an `exhausted` field) over an
  // `existingEntry` that had the map — symmetric data-loss to the one the
  // reset-epoch carry-forward at ccrotate.js:479-483 already defends
  // against. Without this fallback, an account correctly marked exhausted
  // by callMessages keeps getting walked on every selection pass because
  // its `exhausted` map silently disappeared (live thrash 2026-05-20: 5
  // of 13 accounts ended up in this shape; selection burned ~50-74 hits
  // per account per 10 minutes on already-exhausted seats before retrying
  // the one working account). Reconstruct the '*' record from the data
  // still on disk — `exhaustionRecordActive` will still gate on the reset
  // epoch being in the future, so a stale "quota exhausted; resets at
  // <past>" entry won't strand the account.
  if (
...
export function markAccountExhausted(profilesDir, email, { reset5h = null, reset7d = null, response = null, model = null } = {}) {
  ...
  return withCcrotateLock(profilesDir, () => {
    ...
    try {
      ...
      // Distinguish ENOENT (file genuinely absent — safe fresh-install
      // empty fallback) from corruption (file exists but unparseable —
      // a transient cephfs race or interrupted write). The previous
      // catch-all swallow at this seam mass-shrunk the cache in prod
      // on 2026-05-21T01:19Z: tier-cache.json hit a parse-error
      // window, the very next 429 called markAccountExhausted, the
      // empty-fallback wrote a 1-entry file, and the populated 15-
      // entry cache was gone in one atomic rename. Refuse the write
      // on corruption — preserve the on-disk file as-is so the next
      // probe / read cycle can recover it.
      if (e?.code !== 'ENOENT') {
    ...
    if (cacheReadFailed) {
      return {
        skipped: true,
        reason: 'tier-cache unparseable; preserved existing on-disk file (would have mass-shrunk)',
        email,
      };
    }
...
export function clearAccountExhausted(profilesDir, email, { model = null } = {}) {
  return withCcrotateLock(profilesDir, () => {
    ...
    try {
      ...
      // Same distinction as markAccountExhausted: file-missing is a
      // genuine no-op; file-unparseable is a transient corruption and
      // we MUST NOT proceed (the original code returned `no cache file`
      // which is fine for read-only callers, but writers might interpret
      // that as license to write a fresh file and clobber the cache).
      // clearAccountExhausted does not write when cache is absent, so
      // returning a no-op is safe — but we annotate the reason so the
      // shrink-bug RCA path stays visible in logs.
      const reason = e?.code === 'ENOENT' ? 'no cache file' : 'tier-cache unparseable; preserved on-disk file';
      return { changed: false, reason, email };
    }
