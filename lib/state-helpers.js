...
export function markAccountExhausted(profilesDir, email, { reset5h = null, reset7d = null, response = null, model = null } = {}) {
  ...
  return withCcrotateLock(profilesDir, () => {
    ...
    // Only inherit the existing response when that entry was itself
    // exhausted. clearAccountExhausted leaves serviceTier:null +
    // response:'cleared by successful upstream response'; inheriting that
    // here produced self-contradictory rows (serviceTier:'exhausted' with
    // a "cleared" response) — the 14-account "state is off" symptom.
    const inheritedResponse =
      existing?.serviceTier === 'exhausted' ? existing.response : null;

    const entry = {
      email,
      status: 'success',
      serviceTier: 'exhausted',
      response: response || inheritedResponse || fallbackResp,
