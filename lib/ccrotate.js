...
3. Check installation: which claude`);
  ...
  findCodexSessionFileByThreadId(threadId) {
    ...
    return this.listCodexSessionFiles().find(file => file.path.includes(threadId)) || null;
  }
    ...
    for (let index = lines.length - 1; index >= 0; index -= 1) {
      try {
        ...
        if (entry?.type === 'event_msg' && entry?.payload?.type === 'token_count' && rateLimits) {
          return this.normalizeCodexRateSnapshot(rateLimits, entry.timestamp);
        }
      } catch {
  ...
  runCodexProbe(prompt = 'Reply with exactly OK.') {
    ...
    if (result.status !== 0) {
      return {
        status: 'error',
        response: (result.stderr || result.stdout || `codex exec exited with code ${result.status}`).trim()
      };
    }

    const sessionFile = this.findCodexSessionFileByThreadId(threadId)?.path || null;
  ...
  probeCodexAccount(email, accountData, profiles = null) {
    const backup = this.backupCurrentCredentials();

    try {
      this.writeCodexFiles(accountData);
      const probe = this.runCodexProbe();
      const activeAuth = this.readCodexAuth();

      if (profiles && activeAuth) {
        profiles[email] = this.createCodexProfileFromAuth(activeAuth, {
          lastUsed: profiles[email]?.lastUsed || accountData.lastUsed || new Date().toISOString()
        });
      ...
      this.restoreCredentials(backup);
    }
  }
