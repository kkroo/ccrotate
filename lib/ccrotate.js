...
3. Check installation: which claude`);
   ...
   */
  /**
   * Ask the local Claude CLI which account it is authenticated as.
   * `claude auth status` emits JSON: { loggedIn, authMethod, email, ... }.
   * Returns the live session email, or null when it can't be determined
   * (CLI absent, not logged in, malformed output, timeout). A method on the
   * instance so tests can stub it without shelling out.
   */
  async readLiveClaudeAuthEmail() {
    try {
      ...
      if (parsed?.loggedIn === true && typeof parsed.email === 'string' && parsed.email) {
        return parsed.email;
      }
      return null;
    } catch {
