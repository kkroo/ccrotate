...
3. Check installation: which claude`);
  ...
  saveTierCache(results) {
    ...
    // Atomic write: tmpfile + rename. Without this, a concurrent reader
    // on CephFS could observe a truncated file mid-write, and a concurrent
    // writer's interrupted write could leave the cache truncated on disk.
    // The lock-around-RMW pattern in upsertTierCacheEntries also depends
    // on this being atomic from outside its own critical section (e.g.,
    // refresh-one.js's direct saveTierCache call), so we always
    // atomic-rename even when called inside the lock.
    try {
      const tmp = this.tierCacheFile + '.tmp';
      fs.writeFileSync(tmp, JSON.stringify(cache, null, 2), 'utf8');
      fs.renameSync(tmp, this.tierCacheFile);
    } catch { /* non-fatal */ }
  }
