        ...
        const req = https.request({
          ...
          res.on('end', () => {
            if (res.statusCode === 429) {
              ...
              resolve({ status: 'error', response: errorMsg.substring(0, 150), serviceTier: null });
              return;
            }
            ...
            try {
              ...
              if (json.error) {
                resolve({ status: 'error', response: (json.error.message || 'Error').substring(0, 150), serviceTier: null });
                return;
              }
              const serviceTier = json?.usage?.service_tier || null;
              resolve({ status: 'success', response: serviceTier || 'ok', serviceTier });
            } catch {
              resolve({ status: 'error', response: `HTTP ${res.statusCode}`, serviceTier: null });
            }
          });
        });
