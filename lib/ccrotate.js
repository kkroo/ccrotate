...
3. Check installation: which claude`);
  ...
  async testAccount(email) {
    // Uses `claude -p` which routes through the real Claude Code billing path.
    // Returns accurate service_tier. Must run outside an active Claude Code session.
    return new Promise((resolve) => {
      const claudePath = this.findClaudePath();

      const command = `${claudePath} -p "hi" --model haiku --output-format json`;
      ...
      const timeout = setTimeout(() => {
        ...
        resolve({
          status: 'error',
          response: 'Timeout (run from a separate terminal, not inside Claude Code)',
          serviceTier: null,
      ...
      child.on('close', (code) => {
        ...
        try {
          ...
          displayResponse = (serviceTier || json?.result || '').substring(0, 100);
        } catch {
          displayResponse = stdout.trim().substring(0, 100);
        }

        if (!stdout.trim()) {
          resolve({ status: 'error', response: 'No response', serviceTier: null, credentialsUpdated });
          return;
        }

        resolve({ status: 'success', response: displayResponse, serviceTier, credentialsUpdated });
      });

      child.on('error', (error) => {
        clearTimeout(timeout);
        resolve({ status: 'error', response: error.message.substring(0, 150), serviceTier: null, credentialsUpdated: false });
      });
    });
  }
