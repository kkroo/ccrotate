...
3. Check installation: which claude`);
  ...
  writeClaudeFiles(accountData) {
    try {
      ...
      // Read existing config first — before any writes — so the merge sees
      ...
      // Stage BOTH tmp files before publishing either, then rename them
      // back-to-back. This shrinks the window during which a concurrent
      // reader (e.g. a snap on another pod sharing the PVC) could observe
      // an inconsistent (creds=new, config=old) pair from ~milliseconds
      // (the previous order: rename creds → read config → parse → write
      // tmp config → rename) to ~microseconds (one syscall between the
      // two renames). Combined with the snap-clobber pre-snap removal in
      // adapter-claude-k8s 0.2.5-kkroo.3, the observable clobber path is
      // closed end to end. A truly bullet-proof flock-based mutex would
      // require either a new dep (proper-lockfile) or making this async;
      // residual microsecond race acknowledged in
      // memory/feedback_ccrotate_snap_clobber.md.
