...
3. Check installation: which claude`);
   ...
   * Uses /api/oauth/usage (GET, no tokens consumed, rate-limit safe).
   * Returns true if token works, false on 401/403/etc.
   * Returns true on network errors (don't block switch due to connectivity).
   */
  async validateToken(token) {
    ...
    return new Promise((resolve) => {
      ...
      req.on('error', () => resolve(true)); // network issue — don't block
      const t = setTimeout(() => { req.destroy(); resolve(true); }, 5000);
