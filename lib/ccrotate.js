...
3. Check installation: which claude`);
  ...
  async testAccount(email) {
    ...
    // Reads rate limit headers for utilization + tier info. Works inside active sessions.
    try {
      const creds = JSON.parse(fs.readFileSync(this.credentialsFile, 'utf8'));
      const token = creds?.claudeAiOauth?.accessToken;

      if (!token) {
        return { status: 'error', response: 'No access token', serviceTier: null, rateLimits: null, credentialsUpdated: false };
      }
      ...
      const result = await new Promise((resolve) => {
        ...
        const req = https.request({
          ...
          res.on('end', () => {
            ...
            const rateLimits = {
              status: res.headers['anthropic-ratelimit-unified-status'] || null,
              utilization5h: parseFloat(res.headers['anthropic-ratelimit-unified-5h-utilization']) || null,
              utilization7d: parseFloat(res.headers['anthropic-ratelimit-unified-7d-utilization']) || null,
              reset5h: res.headers['anthropic-ratelimit-unified-5h-reset'] || null,
              reset7d: res.headers['anthropic-ratelimit-unified-7d-reset'] || null,
              overageStatus: res.headers['anthropic-ratelimit-unified-overage-status'] || null,
            };

            if (res.statusCode === 429) {
              const errorMsg = (() => {
                try { return JSON.parse(data)?.error?.message || 'Rate limited'; }
                catch { return 'Rate limited'; }
              })();
              resolve({ status: 'error', response: errorMsg.substring(0, 150), serviceTier: null, rateLimits });
              return;
            }
            ...
            try {
              ...
              if (json.error) {
                resolve({ status: 'error', response: (json.error.message || 'Error').substring(0, 150), serviceTier: null, rateLimits });
                return;
              }
              const serviceTier = json?.usage?.service_tier || null;
              const util7d = rateLimits.utilization7d;
              const utilStr = util7d != null ? ` (${Math.round(util7d * 100)}% weekly)` : '';
              resolve({ status: 'success', response: (serviceTier || 'ok') + utilStr, serviceTier, rateLimits });
            } catch {
        ...
        req.on('error', (e) => {
          resolve({ status: 'error', response: e.message.substring(0, 150), serviceTier: null, rateLimits: null });
        });

        const timeout = setTimeout(() => {
          req.destroy();
          resolve({ status: 'error', response: 'Timeout after 10s', serviceTier: null, rateLimits: null });
        }, 10000);
      ...
      return { status: 'error', response: error.message.substring(0, 150), serviceTier: null, rateLimits: null, credentialsUpdated: false };
    }
  }
