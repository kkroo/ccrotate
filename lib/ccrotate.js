...
3. Check installation: which claude`);
  ...
  writeCredentialsToKeychain(credentials, mcpOAuth) {
    // Only called by explicit user actions (switch) that intend to change the active session.
    if (process.platform !== 'darwin') return;

    // Merge mcpOAuth from profile so MCP servers (Linear, Slack, etc.)
    // stay authenticated after account rotation.
    const merged = { ...credentials };
    if (mcpOAuth) {
      merged.mcpOAuth = mcpOAuth;
    }
