...
3. Check installation: which claude`);
  ...
  /**
   * Validate that an access token is accepted by /v1/messages (what Claude Code uses).
   * /api/oauth/usage acceptance does NOT imply /v1/messages acceptance — they have
   * different auth validators. Use the real endpoint to avoid false positives.
   * Sends max_tokens:1 Haiku call — ~2s, ~1 token consumed.
   * Returns true on 200/429, false on 401/403, true on network errors (don't block).
   */
  async validateToken(token) {
    ...
    return new Promise((resolve) => {
      const req = https.request({
        ...
        res.on('data', () => {});
        res.on('end', () => {
          // 200 = works, 429 = rate-limited but token valid, 401/403 = bad token
          if (res.statusCode === 200 || res.statusCode === 429) resolve(true);
          else if (res.statusCode === 401 || res.statusCode === 403) resolve(false);
          else resolve(true); // unknown — don't block
        });
      });
      req.on('error', () => resolve(true));
      const t = setTimeout(() => { req.destroy(); resolve(true); }, 8000);
