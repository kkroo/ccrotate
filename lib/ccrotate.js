...
3. Check installation: which claude`);
   ...
   * Rate-limited to ~1 call/hour per org. Returns null on 429 or error.
   */
   ...
   */
  async testAccount(email, { tryUsageApi = false } = {}) {
    try {
      ...
      // Try per-account usage API if requested (only for first/current account)
      if (tryUsageApi) {
