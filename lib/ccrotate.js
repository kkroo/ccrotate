...
3. Check installation: which claude`);
  ...
  saveProfiles(profiles) {
    ...
    // Hold the cross-process active-files lock so concurrent writers
    // (paperclip-0 lifecycle hooks calling `ccrotate snap` in parallel,
    // or the auth-bot snapping while paperclip-0 saves) serialize their
    // profiles.json publishes. Without this, two writers each stage
    // profiles.json.tmp; whichever renames first wins, the other's
    // rename ENOENTs and its update is lost. See withActiveFilesLock
    // for lockfile semantics.
    try {
  ...
  tierCacheEntryHasRateLimitData(entry) {
    ...
    return rateLimits.utilization5h != null
      ...
      || rateLimits.utilization7dSonnet != null
      || rateLimits.utilization7dOpus != null
  ...
  upsertTierCacheEntries(results) {
    ...
    const existing = this.loadTierCache();
    ...
    for (const result of results) {
      ...
      merged.set(result.email, nextEntry);
    }
  ...
  async snap(force = false, emailOverride = null) {
    return this.commands.snap.execute(force, emailOverride);
  }
   ...
   * if Anthropic's per-org rate limit is currently throttling us, this will
   ...
   * re-hammering the Usage API and re-triggering 429s for the whole pool. */
  ...
  parseUsageResult(usageData, email) {
    ...
    const u7dSonnet = usageData.seven_day_sonnet?.utilization;
    const u7dOpus = usageData.seven_day_opus?.utilization;
    ...
    const reset7dSonnetIso = usageData.seven_day_sonnet?.resets_at;
    const reset7dOpusIso = usageData.seven_day_opus?.resets_at;
    const isoToEpoch = (iso) => (iso ? Math.floor(new Date(iso).getTime() / 1000) : null);
    const reset5h = isoToEpoch(reset5hIso);
    const reset7d = isoToEpoch(reset7dIso);
    const reset7dSonnet = isoToEpoch(reset7dSonnetIso);
    const reset7dOpus = isoToEpoch(reset7dOpusIso);
    ...
    if (u7dSonnet != null) parts.push(`s7d:${Math.round(u7dSonnet)}%`);
    if (u7dOpus != null) parts.push(`o7d:${Math.round(u7dOpus)}%`);
    ...
    return {
      ...
      rateLimits: {
        utilization5h: u5h,
        utilization7d: u7d,
        utilization7dSonnet: u7dSonnet ?? null,
        utilization7dOpus: u7dOpus ?? null,
        reset5h,
        reset7d,
        reset7dSonnet,
        reset7dOpus,
  ...
  async testAccount(email, { tryUsageApi = true, usageApiOnly = false, token: explicitToken } = {}) {
    try {
      ...
      if (!this.cachedExtraUsageEnabled(email)) {
        return { status: 'unknown', response: 'Usage API on cooldown — org-level fallback skipped (no extra usage)', serviceTier: null, credentialsUpdated: false, usageApiWorked: false };
      }
