...
3. Check installation: which claude`);
  ...
  async testAccount(email) {
    // Two-tier approach:
    // 1. Try /api/oauth/usage — per-account, accurate (needs OAuth Bearer + beta header)
    // 2. Fall back to /v1/messages headers — org-level (x-api-key + x-app:cli)
    try {
      ...
      // Try /api/oauth/usage first (per-account usage)
      ...
      if (usageResult) {
        // Got per-account usage data
        const u7d = usageResult.seven_day?.utilization;
        const u5h = usageResult.five_hour?.utilization;
        const extra = usageResult.extra_usage;
        const resetAt = usageResult.seven_day?.resets_at;

        let effectiveTier = 'base';
        if (u7d != null && u7d >= 100) effectiveTier = 'exhausted';
        else if (extra?.is_enabled && extra.used_credits > 0) effectiveTier = 'extra';

        const parts = [];
        if (u5h != null) parts.push(`5h:${Math.round(u5h)}%`);
        if (u7d != null) parts.push(`7d:${Math.round(u7d)}%`);
        if (extra?.used_credits > 0) parts.push(`extra:$${(extra.used_credits / 100).toFixed(2)}`);
      ...
      // Fallback: /v1/messages with x-api-key (org-level headers)
