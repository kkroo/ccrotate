...
3. Check installation: which claude`);
  ...
  async testAccount(email) {
    // Read current credentials (refresh may have switched them)
    try {
      ...
      // Direct Anthropic API call — avoids spawning claude binary (which locks)
      ...
      const result = await new Promise((resolve) => {
          ...
          res.on('end', () => {
            try {
              ...
              if (json.error) {
                ...
                resolve({ status: 'error', response: msg.substring(0, 150), serviceTier: null });
                return;
              }
              ...
              // service_tier may be in usage or top-level
              const serviceTier = json.usage?.service_tier || null;

              resolve({ status: 'success', response: text.substring(0, 100), serviceTier });
            } catch (e) {
              // Rate limit or other non-JSON response
              if (res.statusCode === 429 || res.statusCode === 529) {
                resolve({ status: 'error', response: data.substring(0, 150) || `HTTP ${res.statusCode}`, serviceTier: null });
              } else {
                resolve({ status: 'error', response: `HTTP ${res.statusCode}: ${data.substring(0, 100)}`, serviceTier: null });
              }
            }
          });
        });
        ...
        const timeout = setTimeout(() => {
          req.destroy();
          resolve({ status: 'error', response: 'API timeout after 15 seconds', serviceTier: null });
        }, 15000);
