...
// Usage-API per-token cooldown bounds (seconds). When /api/oauth/usage 429s we
// back off this token's probe and serve last-known-good (`__stale`) until the
// cooldown clears. A flat 1h cooldown (the old default) froze a stale snapshot
// in the tier-cache long after the real 5h/7d window had rolled — live
// 2026-05-30, bot13@blockcast.net showed 5h:100% 7d:79% while Anthropic's UI
// read 32% session / 4% weekly, because the same 429 burst that triggered the
// opus cooldown also parked /api/oauth/usage for a full hour. Shortening the
// window lets the next refresh re-read fresh numbers (and drop the usageStale
// flag) within minutes of recovery instead of up to an hour. We still honor a
// server `retry-after` but cap it, and use a short fallback when it's absent.
const USAGE_API_COOLDOWN_FALLBACK_S = (() => {
  const v = Number(process.env.CCROTATE_USAGE_API_COOLDOWN_S);
  return Number.isFinite(v) && v > 0 ? v : 600; // 10 min
})();
const USAGE_API_COOLDOWN_MAX_S = (() => {
  const v = Number(process.env.CCROTATE_USAGE_API_COOLDOWN_MAX_S);
  return Number.isFinite(v) && v > 0 ? v : 900; // 15 min ceiling on retry-after
})();

/**
 * Compute the usage-API cooldown (ms) for a 429, given the raw `retry-after`
 * header. Honors a positive server value but caps it at USAGE_API_COOLDOWN_MAX_S
 * so an overlong backoff can't freeze a stale snapshot; falls back to
 * USAGE_API_COOLDOWN_FALLBACK_S when the header is missing or unparseable.
 */
export function usageApiCooldownMs(retryAfterHeader) {
  const retryAfter = parseInt(retryAfterHeader, 10);
  const seconds = Number.isFinite(retryAfter) && retryAfter > 0
    ? Math.min(retryAfter, USAGE_API_COOLDOWN_MAX_S)
    : USAGE_API_COOLDOWN_FALLBACK_S;
  return seconds * 1000;
}
...
3. Check installation: which claude`);
  ...
  updateCodexProfile(email, updater, options = {}) {
    if (!email || typeof updater !== 'function') return false;

    this.ensureProfilesDir();
    const file = this.getProfilesFileForTarget('codex');
    let updated = false;
    ...
    withCcrotateLock(this.profilesDir, () => {
      ...
      const existing = profiles[email] || null;
      ...
      profiles[email] = next;
      atomicWriteFileSync(file, JSON.stringify(profiles, null, 2));
      updated = true;
    });
  ...
  upsertCodexProfile(email, profile) {
    return this.updateCodexProfile(email, () => profile, { create: true });
  }

  markCodexProfileStale(email, staleAt = new Date().toISOString()) {
    return this.updateCodexProfile(email, (profile) => ({
      ...profile,
      stale: true,
      staleAt,
    }));
  ...
  clearCodexProfileStale(email, opts = {}) {
    return this.updateCodexProfile(email, (profile) => {
      const next = {
        ...profile,
        stale: false,
      ...
      if (typeof opts.lastApiSyncAt === 'string' && opts.lastApiSyncAt) {
        next.lastApiSyncAt = opts.lastApiSyncAt;
      }
      return next;
    });
  }

  touchCodexProfile(email, opts = {}) {
    const lastUsed = opts.lastUsed || new Date().toISOString();
    return this.updateCodexProfile(email, (profile) => ({
      ...profile,
      lastUsed,
    }));
  ...
  createTierCacheEntry(result, fallbackSyncedAt = null) {
    ...
    return {
      ...
      ...(exhausted ? { exhausted } : {}),
      // Durable workspace-credit exhaustion marker. Must survive into the
      // tier-cache so mergeTierCacheEntry can keep it sticky against
      // credit-blind probes (and clear it only on a creditVerified turn).
      ...(result.exhaustedReason ? { exhaustedReason: result.exhaustedReason } : {}),
      // Credit proof from THIS probe (real codex turn). The merge consumes it
      // to decide whether a usable result may clear an out_of_credits state.
      ...(result.creditVerified === true ? { creditVerified: true } : {}),
      // When the last real credit-verifying turn happened. The freshness loop
      // TTL-gates credit-check probes off this so usable accounts get a real
      // turn only every CREDIT_CHECK_TTL, not every tick.
      ...(result.lastCreditCheckAt ? { lastCreditCheckAt: result.lastCreditCheckAt } : {}),
      // `usageStale` records that the rolling-window percentages below came
      // from fetchAccountUsage's last-known-good cache (the token's
      // /api/oauth/usage is on its own 429 cooldown), NOT a live read. The
      // LKG path still mints a fresh `lastApiSyncAt`, so syncedAt age can't
      // tell stale-from-cooldown apart from a real probe — this explicit flag
      // is the only honest signal. account-table reads it to dim the % column
      // so a frozen "5h:100%" doesn't read as live while Anthropic's own UI
      // shows the window has rolled. Distinct from profile-level `stale`
      // (auth-dead → needs /login). Conditional so a fresh probe's nextEntry
      // omits it and mergeTierCacheEntry (which builds from `...nextEntry`)
      // naturally clears any prior stale flag.
      ...(result.stale ? { usageStale: true } : {}),
      rateLimits,
  ...
  tierCacheEntryHasRateLimitData(entry) {
    // An `exhausted` serviceTier is meaningful data on its own — the signal
    // that an external observer (claude-local writeback, auth-bot) or a
    // real-turn probe captured a runtime quota/credit burn. Check it BEFORE
    // the rateLimits null-guard: the out_of_credits classification from a
    // codex credit-check turn can carry no rolling-window numbers, and if this
    // returned false for it the upsert skip-gate would silently DROP the write
    // (observed live 2026-05-28: a credit-check probe detected omar.ramadan93
    // exhausted but the cache stayed 'available' because the entry had no
    // rateLimits — the freshness loop's whole credit-verify pass was a no-op).
    if (entry?.serviceTier === 'exhausted') return true;
    const rateLimits = entry?.rateLimits || null;
    if (!rateLimits) return false;
    // Reset epochs also count as meaningful data — a refresh that hits Usage
    // API cooldown returns status='unknown' without rolling-window numbers;
    // without this the upsert merge clobbers the exhausted entry.
  ...
  mergeTierCacheEntry(existingEntry, nextEntry) {
    ...
    // Credit exhaustion (`exhaustedReason: 'out_of_credits'`) is INVISIBLE to
    // rate-limit / headers probes — they read x-codex-*-used-percent, never the
    // workspace credit balance. Only a real codex turn (`creditVerified`) proves
    // credits were refilled. So a credit-blind probe (creditVerified !== true)
    // must NOT clear an out_of_credits exhaustion; otherwise the badge flaps back
    // to 🟢 / "usable now" while every routed request fails on credits (live bug
    // 2026-05-27: omar.ramadan93@blockcast.net showed available at 5h:100% while
    // every codex call returned "Your workspace is out of credits"). Absorb the
    // fresh rate-limit telemetry but hold the exhausted classification + reason.
    if (existingEntry.serviceTier === 'exhausted'
        && existingEntry.exhaustedReason === 'out_of_credits'
        && nextEntry.serviceTier !== 'exhausted'
        && nextEntry.creditVerified !== true) {
      return {
        ...nextEntry,
        serviceTier: 'exhausted',
        exhaustedReason: 'out_of_credits',
        response: existingEntry.response ?? nextEntry.response,
      };
    }
  ...
  upsertTierCacheEntries(results) {
    ...
    withCcrotateLock(this.profilesDir, () => {
      ...
      const merged = tierCacheAccountMap(existing);
  ...
  // Workspace credit-balance exhaustion. Distinct from a rate-limit: OpenAI
  // surfaces it on a real turn ("Your workspace is out of credits. Add credits
  // to continue.") with NO 429 and NO change to the x-codex-*-used-percent
  // headers. Because it's invisible to the headers/usage probe, it needs a
  // durable `exhaustedReason: 'out_of_credits'` marker that only a real
  // credit-verified turn can clear (see mergeTierCacheEntry).
  isCodexOutOfCreditsMessage(message = '') {
    const text = String(message || '').toLowerCase();
    return text.includes('out of credits')
      || text.includes('workspace is out of credits')
      || text.includes('add credits to continue');
  }
  ...
  probeCodexAccount(email, accountData, profiles = null) {
    ...
    if (exp != null && exp <= nowSec) {
      if (profiles?.[email] && profiles[email].stale !== true) {
        ...
        profiles[email].staleAt = staleAt;
        try {
          this.markCodexProfileStale(email, staleAt);
        } catch {
    ...
    // Sync probe runs a real `codex exec` turn (runCodexProbe), so a success
    // here is positive proof the workspace credit balance is non-zero —
    // unlike the async path's headers-only fast path. creditVerified: true.
    ...
    if (probe.status !== 'success') {
      ...
      // Real-turn "out of credits" -> durable out_of_credits exhaustion so a
      // later headers-only probe can't re-mark the account usable.
      if (this.isCodexOutOfCreditsMessage(probe.response)) {
        return {
          status: 'success',
          serviceTier: 'exhausted',
          exhaustedReason: 'out_of_credits',
          response: probe.response,
          // "Run out" framing (codex inverse: remaining 0 / utilization 100),
          // mirrors markCodexExhausted so the badge shows 5h:0% AND the upsert
          // skip-gate (tierCacheEntryHasRateLimitData) treats this as data.
          rateLimits: { utilization5h: 100, remaining5h: 0, snapshotCapturedAt: new Date().toISOString() },
          // Real turn just confirmed credit state — stamp it so the freshness
          // loop's TTL gate doesn't immediately re-run another real turn.
    ...
    if (profiles?.[email]) {
      ...
      this.clearCodexProfileStale(email, { lastApiSyncAt });
    }
    ...
    return {
      ...
      creditVerified,
      ...(creditVerified ? { lastCreditCheckAt: new Date().toISOString() } : {}),
      lastApiSyncAt,
      rateLimits: {
   ...
   */
  async probeCodexAccountAsync(email, accountData, profiles = null, opts = {}) {
    const auth = accountData?.auth || null;
    if (!auth) {
      return {
        status: 'error',
    ...
    // creditVerified: only a real codex turn (runCodexProbeAsync runs
    // `codex exec`, which actually generates) proves the workspace credit
    // balance is non-zero. The headers-only fast path reads
    // x-codex-*-used-percent and is BLIND to credits, so its success must
    // never be treated as credit proof — otherwise it clears a genuine
    // out_of_credits exhaustion (see mergeTierCacheEntry).
    //
    // Credit-check mode (opts.creditCheck): SKIP the credit-blind headers
    // fast path and run a real `codex exec` turn directly. This is the only
    // way to catch a headers-"available" account that is actually out of
    // workspace credits (and, symmetrically, to confirm a refill on an
    // already-out_of_credits account — the headers can't see either). The
    // freshness loop schedules this on a TTL so it doesn't burn a real turn
    // every tick. A normal probe keeps the fast path and only falls back to
    // the real turn when headers fail.
    let creditVerified = false;
    let probe;
    if (opts.creditCheck) {
      probe = await this.runCodexProbeAsync(auth);
      if (probe.status === 'success') creditVerified = true;
    } else {
      probe = await this.fetchCodexRateLimitsViaHttp(auth);
      if (probe.status !== 'success') {
        probe = await this.runCodexProbeAsync(auth);
        if (probe.status === 'success') creditVerified = true;
      }
    ...
    if (probe.status !== 'success') {
      ...
      // A real codex turn that failed with "out of credits" is positive
      // evidence the workspace credit balance is zero. Classify it as
      // exhausted with a durable reason (mirrors markCodexExhausted's shape)
      // so a later headers-only probe can't silently re-mark it usable.
      // Without this, the badge shows 🟢/"usable now" while every routed
      // request fails (live bug 2026-05-27: omar.ramadan93@blockcast.net).
      if (this.isCodexOutOfCreditsMessage(probe.response)) {
        return {
          status: 'success',
          serviceTier: 'exhausted',
   ...
   */
  async fetchAccountUsage(token, { ignoreCooldown = false } = {}) {
    ...
    if (!ignoreCooldown && cooldownUntil && Date.now() < cooldownUntil) {
      ...
      const req = https.request({
        ...
        res.on('end', () => {
          if (res.statusCode === 429) {
            // Cache cooldown: honor a (capped) retry-after, else a short
            // fallback — see usageApiCooldownMs. A long flat cooldown used to
            // freeze a stale "5h:100%" snapshot in the tier-cache for an hour.
            cooldowns[tokenKey] = Date.now() + usageApiCooldownMs(res.headers['retry-after']);
  ...
  async testAccount(email, { tryUsageApi = true, usageApiOnly = false, token: explicitToken, ignoreUsageApiCooldown = false } = {}) {
    try {
      ...
      if (tryUsageApi) {
        const usageData = await this.fetchAccountUsage(token, { ignoreCooldown: ignoreUsageApiCooldown });
        if (usageData) {
  ...
  async refreshOne(email) {
    ...
    return this.commands.refreshOne.execute(email);
  }
