...
const RefreshView = ({ accounts, onTestAccount, onComplete, title = '🔄 Testing accounts and refreshing tokens...' }) => {
  ...
  const getTierDisplay = (serviceTier) => {
    if (!serviceTier) return { text: '-', color: 'gray' };
    if (serviceTier === 'standard') return { text: '✅ base', color: 'green' };
    if (serviceTier === 'available') return { text: '✅ available', color: 'green' };
    if (serviceTier === 'near_limit') return { text: '⚠️  near', color: 'yellow' };
    if (serviceTier === 'extra') return { text: '⚠️  extra', color: 'yellow' };
    if (serviceTier === 'exhausted') return { text: '❌ exhausted', color: 'red' };
    return { text: `⚠️  ${serviceTier}`, color: 'yellow' };
  };

  return React.createElement(Box, { flexDirection: 'column', marginTop: 1 },
    React.createElement(Text, { bold: true, color: 'blue' }, title),
    ...
    ...accountStates.map((accountState, index) => {
      ...
      const tierDisplay = getTierDisplay(accountState.serviceTier);
