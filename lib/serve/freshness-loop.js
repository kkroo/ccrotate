 ...
 *     ccrotate.probeCodexAccount, which spawns `codex exec` for a
 *     single-turn probe and reads back rate-limit headers from the
 *     session JSONL. The result is persisted via
 *     ccrotate.upsertTierCacheEntries (codex has no per-profile
 *     `exhausted` map — the tier-cache IS the source of truth, so the
 *     clearExhausted/markExhausted path doesn't apply). A thrown
 *     exception synthesizes an error result but skips the upsert (tool-
 *     level failures aren't valid tier-cache datapoints).
 *
 * Return shape always includes `email`, `status`, `serviceTier`, plus
 * whatever testAccount or probeCodexAccount produced.
 */
export async function probeOne(
  ...
  // api.anthropic.com/api/oauth/usage); codex uses probeCodexAccount
  // (which spawns `codex exec` and reads back rate-limit headers from
  // the session JSONL). PRs #65 and #67 fixed the per-target token
  // and store paths, but left the probe primitive hardcoded to
  // testAccount — a Claude-only endpoint. With a codex bearer that
  // call returns 401, testAccount falls through to
  // `{ status: 'unknown', serviceTier: null }`, and every codex
  // freshness tick logged `tier=? status=unknown`. Codex tier-cache
  // rot was masked because auth-bot's snap-back loop refreshes it on
  // a parallel schedule. Wire the codex probe primitive here so the
  // freshness-loop becomes a real safety net for codex too, matching
  // its role for claude.
  if (target === 'codex') {
    if (!profile?.auth) {
      return {
    ...
    // Distinguish "probeCodexAccount returned an error result"
    // (e.g., expired id_token — a real account-level signal worth
    // persisting) from "probeCodexAccount threw" (a tool-level
    // failure like spawn ENOENT — not a signal about the account).
    // refresh-one --target codex doesn't catch; it lets throws
    // crash the whole refresh. We catch here so a single bad probe
    // doesn't kill the freshness loop, and skip the upsert because
    // a synthesized error from a thrown exception isn't a valid
    // tier-cache datapoint.
    let probe;
    let probeThrew = false;
    try {
      probe = ccrotate.probeCodexAccount(email, profile, profiles);
    } catch (e) {
      probeThrew = true;
      probe = {
        status: 'error',
