...
// How often a usable codex account is re-verified with a real `codex exec`
// turn (credit balance is invisible to the headers probe). TTL-gated so a
// healthy pool isn't real-turned every tick — at most one credit-check probe
// per account per TTL. Override via CCROTATE_CREDIT_CHECK_TTL_MS.
const DEFAULT_CREDIT_CHECK_TTL_MS = 30 * 60_000;
...
function pickStaleEntry(cache, profiles, { staleMinAgeMs, rotationIndex, target = 'claude', creditCheckTtlMs = DEFAULT_CREDIT_CHECK_TTL_MS }) {
  ...
  for (const entry of accounts) {
    ...
    // Codex-only credit verification. The workspace credit balance is
    // INVISIBLE to the headers/usage probe, so a usable account can be
    // silently out-of-credits (and an out_of_credits account silently
    // refilled). For codex we therefore also probe — with a real `codex exec`
    // turn (creditCheck) — (a) accounts flagged out_of_credits, to catch a
    // refill; (b) usable accounts whose last credit-verifying turn is older
    // than the TTL, to catch exhaustion. TTL-gated so a healthy pool isn't
    // real-turned every tick. Claude is unaffected (rate-limit probe sees all).
    let creditCheck = false;
    let isCandidate = isUnusable;
    if (target === 'codex') {
      if (entry?.exhaustedReason === 'out_of_credits') {
        isCandidate = true;
        creditCheck = true;
      } else if (!isUnusable && (status === 'success' || status == null)) {
        const lastCC = parseTimestampMs(entry?.lastCreditCheckAt);
        if (lastCC === 0 || nowMs - lastCC >= creditCheckTtlMs) {
          isCandidate = true;
          creditCheck = true;
        }
      }
    ...
    // Credit-checks bypass the floor (they're TTL-gated above, on a much
    // slower cadence than the snapshot floor).
    const capturedAt = parseTimestampMs(entry?.rateLimits?.snapshotCapturedAt);
    if (!creditCheck && capturedAt > 0 && nowMs - capturedAt < staleMinAgeMs) continue;
    ...
    candidates.push({ email, capturedAt, creditCheck });
  }
...
export async function probeOne(
  ...
  if (target === 'codex') {
    ...
    try {
      // Do not hand the full profiles snapshot to probeCodexAccountAsync here.
      // That helper mutates + saveProfiles(profiles) for Codex stale/success
      // writeback. In serve/state-server mode the snapshot can be several
      // seconds old while a probe is running; saving it wholesale can overwrite
      // concurrent auth-bot quick-refresh or profile-stale/clear writes and
      // reintroduce stale:true on freshly repaired accounts. The freshness loop
      // owns targeted writeback through StateStore below.
      // opts.creditCheck forces a real `codex exec` turn (credit verification)
      // instead of the headers-only fast path — see pickStaleEntry.
      probe = await ccrotate.probeCodexAccountAsync(email, profile, null, { creditCheck: opts.creditCheck === true });
    } catch (e) {
      probeThrew = true;
      probe = {
        status: 'error',
    ...
    if (!probeThrew) {
      ...
      // CRITICAL (HTTP/serve mode): the upsert above wrote THIS serve pod's
      // ephemeral local tier-cache.codex.json — which the canonical
      // state-server (what /ccrotate:when reads, and routing on other replicas
      // reads) never sees. The claude path routes exhaustion through
      // store.markExhausted; the codex path historically wrote only local +
      // the stale flag, so every codex freshness / credit-verify result was
      // invisible to the badge (live 2026-05-28: a credit-check detected
      // omar.ramadan93 exhausted in the pod-local cache but it stayed
      // 'available' canonically — the whole pass was a no-op for the badge).
      // Route an exhausted result to the canonical store. markCodexExhausted
      // tags exhaustedReason:'out_of_credits' when the response says so (#83),
      // so credit exhaustion lands canonically and #83's merge keeps it sticky.
      if (probe?.serviceTier === 'exhausted') {
        try {
          await store.markCodexExhausted?.(email, probe.response ?? null);
        } catch {
          // Non-fatal: the pod-local cache still recorded the result.
        }
      }
      if (probe?.stale === true) {
        try {
          await store.markCodexStale?.(email);
        } catch {
          // Non-fatal: tier-cache still records the stale probe result.
        }
  ...
  async function tick() {
    ...
    try {
      ...
      const result = await probeOne(probeTarget, target.email, ccrotate, store, {
        creditCheck: target.creditCheck === true,
      });
