    ...
    try {
      while (true) {
        ...
        if (item.errorMessage) {
          yield sseError('codex_exec_failed', item.errorMessage);
          continue;
        }
        if (item.done) {
          if (item.code !== 0) {
            const parsed = parseCodexJsonOutput([stdout, stderr].filter(Boolean).join('\n'));
            const message = parsed.errorMessage || stderr.trim() || `codex exec exited with code ${item.code}`;
            yield sseError(
              isCodexUsageLimitMessage(message) ? 'insufficient_quota' : 'codex_exec_failed',
              message,
              isCodexUsageLimitMessage(message) ? 'insufficient_quota' : 'server_error',
            );
          } else {
...
async function callResponsesViaCodexExecStream(payload, { timeoutMs = 15 * 60_000 } = {}) {
  ...
  const startEmail = readCodexActiveEmail(dir, profiles);
  const tried = new Set();
  const cur = startEmail && !profiles[startEmail]?.stale && getCodexAuth(profiles[startEmail])
    ? { email: startEmail, profile: profiles[startEmail] }
    : pickNextCodex(profiles, tried);
  if (!cur) {
    return {
      status: 502,
      response: errorFetchResponse(502, 'pool_exhausted', 'No Codex accounts with usable auth.'),
      attempts: 0,
      poolExhausted: true,
    };
  }
  const auth = getCodexAuth(cur.profile);
  const result = runCodexExecSse({ auth, payload, timeoutMs });
  return { ...result, attempts: 1, account: cur.email };
}
