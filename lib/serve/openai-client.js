...
function pickNextCodexAccess(profiles, tried) {
  for (const [email, prof] of Object.entries(profiles)) {
    if (tried.has(email)) continue;
    if (prof?.stale) continue;
...
async function callResponsesViaChatGptCodexProxy(payload, {
  ...
  const tried = new Set();
  let cur = startEmail && !profiles[startEmail]?.stale && getCodexAccessToken(profiles[startEmail])
    ? { email: startEmail, profile: profiles[startEmail] }
    : pickNextCodexAccess(profiles, tried);
  let attempts = 0;
  let last = null;
  ...
  while (cur) {
    ...
    if (!accessToken) {
      cur = pickNextCodexAccess(profiles, tried);
      if (cur) await store.setActiveEmail(cur.email);
      continue;
    }
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      const isSse = String(response.headers.get('content-type') || '').includes('text/event-stream');
      return {
        status: response.status,
        response,
        stream: stream && isSse && response.body ? response.body : undefined,
    ...
    if (response.status === 429 || response.status === 401 || response.status === 403) {
      ...
      if (failure.quota) {
        ...
        const next = pickNextCodexAccess(profiles, tried);
        if (next) {
          await store.setActiveEmail(next.email);
          cur = next;
          continue;
