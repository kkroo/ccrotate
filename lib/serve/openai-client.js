...
//     chat completions when a future API-scoped token is available.
...
function sseError(code, message, type = 'server_error') {
  return sseData({ type: 'error', error: { type, code, message } });
}
...
function runCodexExecSse({ auth, payload, timeoutMs }) {
  const tempHome = createTempCodexHome(auth);
  const prompt = responsesInputToPrompt(payload);
  const { response: skeleton, msgId } = makeResponseSkeleton(payload);
  ...
  async function* stream() {
    ...
    yield sseData({ type: 'response.created', response: skeleton });
    yield sseData({
      type: 'response.output_item.added',
      response_id: skeleton.id,
      output_index: 0,
    ...
    const pushLine = (line) => {
      ...
      try {
        ...
        if (entry?.type === 'item.completed' && entry.item?.type === 'agent_message') {
          ...
          push({ errorMessage: entry.message });
        } else if (entry?.type === 'turn.failed' && typeof entry.error?.message === 'string') {
          push({ errorMessage: entry.error.message });
        }
      } catch {
        // Ignore non-JSON lines emitted by wrappers or warnings.
      }
    ...
    child.stderr.on('data', (chunk) => { stderr += chunk; });
    child.on('error', (error) => push({ errorMessage: error.message }));
    child.on('close', (code) => {
      finished = true;
      clearTimeout(timer);
      if (stdoutRemainder.trim()) pushLine(stdoutRemainder);
      push({ done: true, code });
    });
    ...
    try {
      while (true) {
        if (queue.length === 0) {
          ...
          await Promise.race([wait, new Promise(resolve => setTimeout(resolve, 10_000))]);
        }
        if (queue.length === 0) {
          if (Date.now() - lastHeartbeat >= 10_000) {
            lastHeartbeat = Date.now();
            yield ': ccrotate-serve keepalive\n\n';
          }
          continue;
        }
        ...
        if (item.done) {
          if (item.code !== 0) {
            const parsed = parseCodexJsonOutput([stdout, stderr].filter(Boolean).join('\n'));
            const message = parsed.errorMessage || lastErrorMessage || stderr.trim() || `codex exec exited with code ${item.code}`;
            const quota = isCodexUsageLimitMessage(message);
            const code = quota ? 'insufficient_quota' : 'codex_exec_failed';
            const type = quota ? 'insufficient_quota' : 'server_error';
            const response = failedResponse(skeleton, code, message, type);
            yield sseData({ type: 'response.failed', response });
          } else {
            const finalUsage = codexUsageToResponsesUsage(usage);
            const finalResponse = {
              ...skeleton,
              status: 'completed',
              output: [{
            ...
            yield sseData({ type: 'response.completed', response: finalResponse });
          }
          yield 'data: [DONE]\n\n';
          break;
        }
...
// /v1/images/generations — DALL-E / gpt-image-1 passthrough. Same shape as
// callEmbeddings (JSON in, JSON out, no streaming, no Claude translation).
// Image generation has no Codex-exec equivalent, so the dispatch fallback
// would just fail; the OPENAI_API_KEY branch is the only viable path. The
// caller's API key must carry the appropriate image-generation scope —
// embeddings-only keys 401 here exactly as they do elsewhere.
export async function callImages(payload, opts = {}) {
  const { timeoutMs = 120000 } = opts;
  if (process.env.OPENAI_API_KEY) {
    const response = await callOnceJson({ url: IMAGES_URL, payload, timeoutMs });
    return { status: response.status, response, attempts: 1 };
  }
  throw new Error('openai-client: callImages requires OPENAI_API_KEY (no Codex-exec fallback for /v1/images/generations)');
}
