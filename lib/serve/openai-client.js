...
//     /v1/responses without caller tools is bridged through `codex exec` with
//     the selected saved Codex auth. Public OpenAI API endpoints reject
//     ChatGPT-scoped Codex OAuth tokens with invalid_claims.
//
//     Tool-bearing /v1/responses requests must not use that bridge: nesting
//     `codex exec` moves the tool loop into the ccrotate-serve pod, so local
//     callers see shell/network commands run remotely. We also do not fall
//     back to OPENAI_API_KEY here; that key is reserved for embeddings.
...
function hasCallerTools(payload = {}) {
  return Array.isArray(payload.tools) && payload.tools.length > 0;
}

function codexResponsesMode() {
  return String(process.env.CCROTATE_CODEX_RESPONSES_MODE || 'auto').trim().toLowerCase();
}
...
export async function callResponses(payload, opts = {}) {
  if (process.env.CCROTATE_CODEX_DIR) {
    if (hasCallerTools(payload) && codexResponsesMode() !== 'exec') {
      return {
        status: 400,
        response: errorFetchResponse(
          400,
          'local_tools_unsupported',
          'ccrotate-serve cannot preserve caller-local tool execution through the Codex subscription bridge, and OPENAI_API_KEY is reserved for embeddings. Set CCROTATE_CODEX_RESPONSES_MODE=exec only if you explicitly want server-side tool execution in the ccrotate-serve pod.',
        ),
        attempts: 0,
      };
    }
