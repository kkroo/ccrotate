...
// /v1/images/generations — DALL-E / gpt-image-1 passthrough. ALWAYS routes
// through the Codex/ChatGPT account pool — never via OPENAI_API_KEY (org
// Platform keys are embeddings-scoped and can't do image generation).
// Bypasses dispatch's OPENAI_API_KEY fallback and goes straight to the
// Codex path. Errors clearly when no Codex bank is configured.
export async function callImages(payload, opts = {}) {
  ...
  return callOpenaiViaCodex(IMAGES_URL, payload, { timeoutMs });
}
