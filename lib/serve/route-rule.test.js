...
describe('listModels', () => {
  it('includes only configured backends', () => {
    ...
    expect(both.models).toBe(both.data);
    expect(both.data.every(m => m.slug === m.id)).toBe(true);
    expect(both.data.every(m => m.display_name === m.id)).toBe(true);
    expect(both.data.some(m => m.owned_by === 'anthropic')).toBe(true);
    expect(both.data.some(m => m.owned_by === 'openai')).toBe(true);
    expect(both.data.some(m => m.id === 'claude-opus-4-7[1m]')).toBe(true);
    expect(both.data.some(m => m.id === 'claude-sonnet-4-6[1m]')).toBe(false);
    expect(both.data.some(m => m.id === 'claude-haiku-4-5-20251001[1m]')).toBe(false);
  ...
  it('returns empty data when no backend is configured', () => {
    const empty = listModels({ hasAnthropic: false, hasOpenai: false });
    expect(empty).toEqual({ object: 'list', data: [], models: [] });
    expect(empty.models).toBe(empty.data);
  });

  it('every advertised anthropic model routes to anthropic via pickUpstream', () => {
    for (const id of ANTHROPIC_MODEL_ALIASES) {
