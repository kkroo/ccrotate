...
export class FileStateStore {
  ...
  async markCrossWritten(email, detectedIdentity, offendingAccessToken = null) {
...
export class HttpStateStore {
  ...
  async markCrossWritten(email, detectedIdentity, offendingAccessToken = null) {
    const r = await this._request('/state/cross-wired', {
      method: 'POST',
      body: JSON.stringify({ email, detectedIdentity, offendingAccessToken }),
    });
    this._invalidate('profiles');
    return r;
  }
