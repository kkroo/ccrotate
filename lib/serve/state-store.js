...
//   markAnthropicStale(email, { reason, response }) -> result
//   clearAnthropicStale(email, { lastApiSyncAt }) -> result
...
import { validTierCacheAccounts } from '../tier-cache-rows.js';
import {
...
export class FileStateStore {
  ...
  // Mark an Anthropic profile stale (organization disabled Claude Code
  // subscription access, etc.). Mirror of markCodexStale on the Codex
  // pool. Writes profiles.json (NOT profiles.codex.json). The rotator's
  // pickNextCandidate skips profiles where `stale === true`; the access
  // token is preserved (admin re-enable is the recovery path, not a
  // relogin), so an operator can clearAnthropicStale once access is
  // restored.
  //
  // The `reason` and `response` opts are captured for forensics + the
  // operator-facing badge — they don't change rotator behavior.
  async markAnthropicStale(email, opts = {}) {
    let updated = false;
    withCcrotateLock(this.profilesDir, () => {
      ...
      if (profiles[email]) {
        ...
        if (typeof opts.reason === 'string' && opts.reason) {
          profiles[email].staleReason = opts.reason;
        }
        if (typeof opts.response === 'string' && opts.response) {
          // Truncate to keep the on-disk record bounded — full body is
          // also in the request-attribution log via callMessages.
          profiles[email].staleResponse = String(opts.response).slice(0, 480);
        }
        const tmp = file + '.tmp';
  ...
  async clearAnthropicStale(email, opts = {}) {
    let updated = false;
    withCcrotateLock(this.profilesDir, () => {
      const file = path.join(this.profilesDir, 'profiles.json');
      let profiles;
      ...
      const before = JSON.stringify({
        ...
        staleReason: profiles[email].staleReason ?? null,
        staleResponse: profiles[email].staleResponse ?? null,
        lastApiSyncAt: profiles[email].lastApiSyncAt ?? null,
      });
      profiles[email].stale = false;
      delete profiles[email].staleAt;
      delete profiles[email].staleReason;
      delete profiles[email].staleResponse;
  ...
  // Persist a token rotated by `codex exec` back onto the codex profile.
  // codex refreshes the OAuth pair in CODEX_HOME/auth.json on use and the old
  // refresh token is invalidated upstream; without this writeback the stored
  // token goes dead and the account hard-fails until a manual /login + snap.
  // Only the auth blob is touched (mirrors the file shape `profiles[email].auth`).
  async saveCodexAuth(email, auth) {
    let updated = false;
    if (!email || !auth) return { updated };
    withCcrotateLock(this.profilesDir, () => {
      const file = path.join(this.profilesDir, 'profiles.codex.json');
      const profiles = JSON.parse(fs.readFileSync(file, 'utf8'));
      if (profiles[email]) {
        profiles[email].auth = auth;
        const tmp = file + '.tmp';
        fs.writeFileSync(tmp, JSON.stringify(profiles, null, 2));
        fs.renameSync(tmp, file);
        updated = true;
  ...
  async markCodexExhausted(email, response = null) {
    withCcrotateLock(this.profilesDir, () => {
      ...
      const accounts = validTierCacheAccounts(cache);
      const existing = accounts.find(a => a.email === email);
      cache.accounts = accounts.filter(a => a.email !== email);
...
export class HttpStateStore {
  ...
  async saveCodexAuth(email, auth) {
    if (!email || !auth) return { updated: false };
    const r = await this._request('/state/profile-codex-auth', {
      method: 'POST',
