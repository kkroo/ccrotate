...
describe('FileStateStore', () => {
  ...
  it('markCodexExhausted tags out_of_credits distinctly; rate-limit stays untagged', async () => {
    const store = new FileStateStore(dir);
    await store.markCodexExhausted('credit@x.net', 'Your workspace is out of credits. Add credits to continue.');
    await store.markCodexExhausted('rl@x.net', 'You have hit your usage limit. Try again at 12:00 PM.');
    const cache = read('tier-cache.codex.json');
    const credit = cache.accounts.find(a => a.email === 'credit@x.net');
    const rl = cache.accounts.find(a => a.email === 'rl@x.net');
    // out_of_credits gets the durable, headers-blind marker...
    expect(credit.serviceTier).toBe('exhausted');
    expect(credit.exhaustedReason).toBe('out_of_credits');
    // ...while a plain rate-limit exhaustion does not (keeps normal recovery).
    expect(rl.serviceTier).toBe('exhausted');
    expect(rl.exhaustedReason).toBeUndefined();
  });
  ...
  it('does not publish a cooldown for a 429 without rate-limit bucket headers', async () => {
    const store = new FileStateStore(dir);
    await store.recordAnthropicRateLimit(
      'a@x.net',
      'claude-opus-4-5',
    ...
    const state = read('rate-limit-state.json');
    const entry = state.anthropic.accounts['a@x.net'].modelGroups['claude-opus'];
    ...
    expect(entry.last429Reason).toBe('unknown');
    expect(await store.getAnthropicRateLimitBlock('a@x.net', 'claude-opus-4-5')).toMatchObject({
      blocked: false,
  ...
  it('clears Anthropic rate-limit state for an account or model group', async () => {
    write('rate-limit-state.json', {
      version: 1,
      updatedAt: 'old',
      anthropic: {
        accounts: {
    ...
    const store = new FileStateStore(dir);
    const one = await store.clearAnthropicRateLimitState('a@x.net', { model: 'claude-opus-4-7' });
    expect(one.cleared).toBe(true);
    expect(read('rate-limit-state.json').anthropic.accounts['a@x.net'].modelGroups['claude-opus']).toBeUndefined();
    expect(read('rate-limit-state.json').anthropic.accounts['a@x.net'].modelGroups['claude-sonnet']).toBeTruthy();

    const all = await store.clearAnthropicRateLimitState('a@x.net');
  ...
  it('writeProfileToken updates the OAuth triple in place', async () => {
    ...
    expect(read('rate-limit-state.json').anthropic.accounts['a@x.net']).toBeUndefined();
  });

  it('markCrossWritten clears stale Anthropic rate-limit state for the profile', async () => {
    write('profiles.json', SAMPLE_PROFILES);
    write('rate-limit-state.json', {
      version: 1,
      updatedAt: 'old',
    ...
    const result = await new FileStateStore(dir)
      .markCrossWritten('a@x.net', 'other@x.net', 'tok-a');

    expect(result.updated).toBe(true);
    expect(read('profiles.json')['a@x.net'].credentials.claudeAiOauth.accessToken).toBe(null);
    expect(read('rate-limit-state.json').anthropic.accounts['a@x.net']).toBeUndefined();
  });
  ...
  it('clearCodexStale clears the codex profile stale flag', async () => {
    write('profiles.codex.json', { 'c@x.net': { auth: {}, stale: true, staleAt: 'old' } });
    const res = await new FileStateStore(dir).clearCodexStale('c@x.net', { lastApiSyncAt: 'sync' });
    expect(res.updated).toBe(true);
    const prof = read('profiles.codex.json')['c@x.net'];
  ...
  it('markAnthropicStale flags the anthropic profile stale and records reason+response', async () => {
    write('profiles.json', { 'a@x.net': { credentials: { claudeAiOauth: { accessToken: 'at' } } } });
    const res = await new FileStateStore(dir).markAnthropicStale('a@x.net', {
      reason: 'organization_disabled',
      response: 'Your organization has disabled Claude Code subscription access.',
    });
    expect(res.updated).toBe(true);
    const prof = read('profiles.json')['a@x.net'];
    expect(prof.stale).toBe(true);
    expect(typeof prof.staleAt).toBe('string');
    expect(prof.staleReason).toBe('organization_disabled');
    expect(prof.staleResponse).toContain('disabled Claude Code subscription access');
    // Access token is preserved — admin re-enable is the recovery path,
    // not a relogin.
    expect(prof.credentials.claudeAiOauth.accessToken).toBe('at');
  });

  it('markAnthropicStale truncates a very long response to bound the on-disk record', async () => {
    write('profiles.json', { 'a@x.net': { credentials: {} } });
    const long = 'x'.repeat(2000);
    await new FileStateStore(dir).markAnthropicStale('a@x.net', { reason: 'r', response: long });
    expect(read('profiles.json')['a@x.net'].staleResponse.length).toBe(480);
  });

  it('clearAnthropicStale clears the anthropic profile stale flag + companion fields', async () => {
    write('profiles.json', {
      'a@x.net': {
        credentials: {},
        stale: true,
    ...
    const res = await new FileStateStore(dir).clearAnthropicStale('a@x.net', { lastApiSyncAt: 'sync' });
    expect(res.updated).toBe(true);
    const prof = read('profiles.json')['a@x.net'];
    expect(prof.stale).toBe(false);
    expect(prof.staleAt).toBeUndefined();
    expect(prof.staleReason).toBeUndefined();
    expect(prof.staleResponse).toBeUndefined();
    expect(prof.lastApiSyncAt).toBe('sync');
  });

  it('saveCodexAuth persists a rotated token blob onto the codex profile', async () => {
    // Original profile has the token codex was launched with...
    write('profiles.codex.json', {
      'c@x.net': {
        provider: 'codex',
        auth: { tokens: { refresh_token: 'OLD', access_token: 'oldA' }, last_refresh: 't0' },
        name: 'C',
      },
    });
    // ...and codex rotated it on use. Persist the new blob back.
    const rotated = { tokens: { refresh_token: 'NEW', access_token: 'newA' }, last_refresh: 't1' };
    const res = await new FileStateStore(dir).saveCodexAuth('c@x.net', rotated);
    expect(res.updated).toBe(true);
    const prof = read('profiles.codex.json')['c@x.net'];
    expect(prof.auth).toEqual(rotated);
    // Non-auth fields are untouched.
    expect(prof.name).toBe('C');
    expect(prof.provider).toBe('codex');
  });

  it('saveCodexAuth is a no-op for an unknown email (no crash, no write)', async () => {
    write('profiles.codex.json', { 'c@x.net': { auth: { tokens: { refresh_token: 'OLD' } } } });
    const res = await new FileStateStore(dir).saveCodexAuth('missing@x.net', { tokens: { refresh_token: 'NEW' } });
    expect(res.updated).toBe(false);
    expect(read('profiles.codex.json')['c@x.net'].auth.tokens.refresh_token).toBe('OLD');
  });

  it('saveCodexAuth ignores empty email / auth', async () => {
    write('profiles.codex.json', { 'c@x.net': { auth: {} } });
    const store = new FileStateStore(dir);
    expect((await store.saveCodexAuth('', { tokens: {} })).updated).toBe(false);
    expect((await store.saveCodexAuth('c@x.net', null)).updated).toBe(false);
  });
  ...
  it('markCodexExhausted skips null or malformed tier-cache rows', async () => {
    write('tier-cache.codex.json', {
      updatedAt: 'old',
      accounts: [
        null,
        {},
        ...
        { email: 'c@x.net', serviceTier: 'available', response: 'old' },
      ],
    });
    await new FileStateStore(dir).markCodexExhausted('c@x.net', 'usage limit');
    ...
    expect(cache.accounts.find(a => a.email === 'c@x.net').serviceTier).toBe('exhausted');
  });
...
describe('HttpStateStore', () => {
  ...
  it('clearAnthropicRateLimitState POSTs /state/rate-limits/anthropic/clear', async () => {
    const spy = mockFetch(() => json({ cleared: true, email: 'a@x.net' }));
    const res = await new HttpStateStore({ baseUrl: 'http://s:4002' })
      .clearAnthropicRateLimitState('a@x.net', { model: 'claude-opus-4-7' });
    expect(res.cleared).toBe(true);
    const [url, init] = spy.mock.calls[0];
    expect(url).toBe('http://s:4002/state/rate-limits/anthropic/clear');
    expect(JSON.parse(init.body)).toMatchObject({ email: 'a@x.net', model: 'claude-opus-4-7' });
  });
  ...
  it('clearCodexStale POSTs /state/profile-stale/clear with target codex', async () => {
    const spy = mockFetch(() => json({ updated: true }));
    await new HttpStateStore({ baseUrl: 'http://s:4002' })
      .clearCodexStale('c@x.net', { lastApiSyncAt: 'sync' });
    const [url, init] = spy.mock.calls[0];
    expect(url).toBe('http://s:4002/state/profile-stale/clear');
  ...
  it('markAnthropicStale POSTs /state/profile-stale with target claude + reason/response', async () => {
    const spy = mockFetch(() => json({ updated: true }));
    await new HttpStateStore({ baseUrl: 'http://s:4002' })
      .markAnthropicStale('a@x.net', { reason: 'organization_disabled', response: 'disabled' });
    const [url, init] = spy.mock.calls[0];
    expect(url).toBe('http://s:4002/state/profile-stale');
    expect(JSON.parse(init.body)).toEqual({
      email: 'a@x.net',
  ...
  it('clearAnthropicStale POSTs /state/profile-stale/clear with target claude', async () => {
    const spy = mockFetch(() => json({ updated: true }));
    await new HttpStateStore({ baseUrl: 'http://s:4002' })
      .clearAnthropicStale('a@x.net', { lastApiSyncAt: 'sync' });
    const [url, init] = spy.mock.calls[0];
    expect(url).toBe('http://s:4002/state/profile-stale/clear');
    expect(JSON.parse(init.body)).toEqual({
      email: 'a@x.net',
