...
function prepareAnthropicRequest(payload) {
  ...
  // The Messages API deprecated top-level `output_format` (now 400s) in favor
  // of `output_config.format`. Rewrite it so older clients — notably the
  // security-guidance plugin — keep working through the proxy. Prefer a
  // client-supplied `output_config.format` if present; otherwise fold the
  // deprecated field into any existing `output_config` (e.g. {effort}).
  if (upstreamPayload.output_format !== undefined) {
    const deprecatedFormat = upstreamPayload.output_format;
    delete upstreamPayload.output_format;
    const existing = (upstreamPayload.output_config && typeof upstreamPayload.output_config === 'object'
      && !Array.isArray(upstreamPayload.output_config))
      ? upstreamPayload.output_config
      : {};
    upstreamPayload.output_config = ('format' in existing)
      ? existing
      : { ...existing, format: deprecatedFormat };
  }
...
// Detect Claude-Code subscription-disabled responses. Anthropic returns a
// 403 + permission_error when an org admin has disabled Claude-Code
// subscription access for the seat. Body looks like:
//
//   {"type":"error","error":{"type":"permission_error","message":
//    "Your organization has disabled Claude Code subscription access. ..."}}
//
// The CLI then renders the message in the TUI (joined by " · " with a
// "Use an Anthropic API key instead" follow-up). Without this match, the
// 403 falls through to the non-401 success-return path in callMessages —
// the account is marked active and every subsequent request 403s on the
// same dead seat instead of rotating.
//
// Differs from `out_of_credits` (Codex side, openai-client.js#L148): an
// org-disabled state isn't a quota condition. There's no reset horizon —
// only an admin can re-enable. We mark the profile `stale` (mirror of
// markCodexStale) so the rotator skips it AND an operator can see the
// flag and clear it once access is restored.
function isOrganizationDisabledMessage(body) {
  ...
  if (type !== 'permission_error') return false;
  return (
    message.includes('subscription access for claude code') ||
    message.includes('subscription access')              ||  // future-proof against minor msg drift
    message.includes('use an anthropic api key instead') ||
    (message.includes('organization has disabled') && message.includes('claude'))
  );
}
export { isOrganizationDisabledMessage };
...
export async function pickNextCandidate(store, alreadyTried, model = null) {
  ...
  if (activeEmail && !alreadyTried.has(activeEmail)) {
    ...
    if (
      !exhaustedSet.has(activeEmail) &&
      prof?.credentials?.claudeAiOauth?.accessToken &&
      prof?.stale !== true
    ) {
  ...
  for (const [email, prof] of Object.entries(profiles)) {
    ...
    // Skip profiles flagged stale (e.g. by markAnthropicStale after a 403
    // "organization disabled Claude Code subscription access"). Mirrors
    // the markCodexStale path on the Codex pool — the rotator must not
    // burn requests on an admin-disabled seat until an operator clears
    // the flag.
    if (prof.stale === true) continue;
...
export async function callMessages(payload, opts = {}) {
  ...
  const checkCrossWired = async (email, accessToken) => {
    ...
    try {
      ...
      try {
        const quarantine = await store.markCrossWritten?.(email, probed.email, accessToken);
        if (quarantine?.reason === 'token rotated past offending one') {
          try {
            const freshProfiles = await store.getProfiles?.();
            const freshToken = freshProfiles?.[email]?.credentials?.claudeAiOauth?.accessToken;
            if (freshToken && freshToken !== accessToken) {
              const freshProbe = await probeOauthProfile(freshToken, { timeoutMs: usageApiTimeoutMs });
              if (freshProbe?.email && freshProbe.email.toLowerCase() !== email.toLowerCase()) {
                console.log(
                  `[callMessages] CROSS-WRITE STILL PRESENT AFTER ROTATION: profile=${email} now holds token for ${freshProbe.email}${freshProbe.stale ? ' (stale)' : ''} — quarantining current token`,
                );
                await store.markCrossWritten?.(email, freshProbe.email, freshToken);
              }
            }
          } catch (recheckErr) {
            console.log(`[callMessages] cross-write post-rotation recheck threw: ${recheckErr?.message ?? recheckErr}`);
          }
        }
      } catch (qe) {
  ...
  let organizationDisabledResponse = null;
  let attempts = 0;
  let trigger = null;
  // No usable account at entry: the whole Claude pool is exhausted/throttled
  // for this model or pending relogin, so pickNextCandidate returned null.
  // Return a synthesized retryable 429 — mirroring the pool-exhausted return
  // branches below — instead of dereferencing a null candidate in the
  // request_start log. The pre-fix `cand.email` threw a TypeError that the
  // serve handler mapped to a fatal HTTP 500 (BLO-8611); the codex sibling
  // already guards this path, the Claude path did not.
  if (!cand) {
    logAttribution('request_complete', attribution, {
      account: null,
      status: 429,
      attempts: 0,
      trigger: 'no-usable-account',
    ...
    return {
      ...
      response: new Response(JSON.stringify({
        error: {
          ...
          message: 'No ccrotate Anthropic account is currently usable (all exhausted/throttled or pending relogin). Retry shortly.',
        },
      }), {
        status: 429,
        headers: { 'content-type': 'application/json' },
  ...
  while (cand) {
    ...
    // 403 + permission_error "organization has disabled Claude Code
    // subscription access" → admin policy state, not a quota condition.
    // Mark the profile stale (operator-clearable) and rotate. Without
    // this branch the 403 falls through to the non-401 success path
    // below, the account is set active, and every subsequent request
    // hammers the same dead seat instead of rotating. See
    // isOrganizationDisabledMessage for the full body shape and rationale.
    if (response.status === 403) {
      let body = null;
      try { body = await response.clone().json(); } catch {}
      if (isOrganizationDisabledMessage(body)) {
        const reason = body?.error?.message || 'organization disabled Claude Code subscription access';
        console.log(
          `[callMessages] account=${cand.email} 403 organization-disabled — marking stale + rotating (msg="${String(reason).slice(0, 140)}")`,
        );
        logAttribution('organization_disabled', attribution, {
          account: cand.email,
          attempt: attempts,
          status: response.status,
        ...
        try {
          await store.markAnthropicStale?.(cand.email, { reason: 'organization_disabled', response: reason });
        } catch (err) {
          console.log(`[callMessages] markAnthropicStale failed for ${cand.email}: ${err?.message ?? err}`);
        }
        trigger = 'organization-disabled';
        lastResponse = response;
        organizationDisabledResponse = response;
        cand = await pickNextCandidate(store, tried, payload.model);
        continue;
      }
    }
  ...
  // Pool walked through one or more organization-disabled 403s and ran
  // out of candidates. Return the upstream 403 verbatim (with its body)
  // instead of synthesizing a 502 — callers (paperclip wrapper, claude-
  // cli, …) drive retry/backoff decisions off the status. A 502 would
  // be treated as transient and retried; a 403 surfaces the policy
  // state correctly.
  if (organizationDisabledResponse) {
    logAttribution('request_complete', attribution, {
      account: null,
      status: 403,
      attempts,
    ...
    return {
      status: 403,
      response: organizationDisabledResponse,
      attempts,
      account: null,
      trigger: 'organization-disabled',
      poolExhausted: true,
