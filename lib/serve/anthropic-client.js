...
function prepareAnthropicRequest(payload) {
  if (payload?.model !== 'claude-opus-4-7[1m]') {
    return { payload, headers: HEADERS_TEMPLATE };
  }
  return {
    payload: { ...payload, model: 'claude-opus-4-7' },
    headers: {
      ...HEADERS_TEMPLATE,
...
function classifyQuotaError(body, responseHeaders) {
  ...
  // Spec-original "transient vs structural" distinction proved fragile in
  // production — Anthropic's actual 429 message is "This request would exceed
  // your account's rate limit..." which matched none of the original patterns.
  // We now treat ANY rate_limit_error from Anthropic as structural, because:
  //   1) Anthropic only emits 429 when there's a real upstream condition.
  //   2) Our pool has many accounts; rotation cost is bounded by replay-once cap.
  //   3) Reset epoch parsed from `anthropic-ratelimit-*-reset` headers when present.
  ...
  if (responseHeaders) {
    ...
    reset5h = hdr('anthropic-ratelimit-tokens-reset') ?? hdr('retry-after') ?? null;
  }
  if (reset5h == null) {
...
function setActiveOnSuccess(profilesDir, email, response) {
...
export async function callMessages(payload, opts = {}) {
  ...
  while (cand) {
    ...
    if (response.status !== 401) {
      setActiveOnSuccess(profilesDir, cand.email, response);
