...
async function pickNextCandidate(store, alreadyTried, model = null) {
  const profiles = await store.getProfiles();
...
export async function callMessages(payload, opts = {}) {
  ...
  // tests inject `opts.transient429BackoffMs` / `opts.sleep`.
  const transient429BackoffMs = opts.transient429BackoffMs
    ?? Number(process.env.CCROTATE_TRANSIENT_429_BACKOFF_MS ?? 400);
  ...
  while (cand) {
    ...
    if (response.status === 429) {
      ...
      // (observed live 2026-05-19: 190 transient vs 74 structural, requests
      // walking 14-deep). A short jittered gap lets the token bucket
      // recover so a later attempt can land a 200. Structural 429s skip
      // this — that account is genuinely capped. ±25% jitter decorrelates
      // concurrent requests so they don't re-sync into a fresh burst.
