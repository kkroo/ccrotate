...
export async function callMessages(payload, opts = {}) {
  ...
  // 1-5s upstream burst. Test/disable knob via opts.waitOnTransient429MaxSec
  ...
  const tried = new Set();
  // Per-request: emails we've already waited-then-retried on. Caps the
  // wait-on-same-account behavior to ONE wait per account per request so
  // a stubbornly throttled account can't block the request forever; the
  // second 429 from the same account falls through to the rotate path.
  const waitedAccounts = new Set();
  ...
  while (cand) {
    ...
    try {
      ...
      if (block?.blocked && !waitedAccounts.has(cand.email)) {
        console.log(
    ...
    if (response.status === 429) {
      ...
      const canWait =
        ...
        !waitedAccounts.has(cand.email);
      if (canWait) {
        waitedAccounts.add(cand.email);
        const waitMs = (retryAfterSec + 1) * 1000;
        console.log(
          `[callMessages] account=${cand.email} transient 429 retry-after=${retryAfterSec}s — waiting on same account instead of rotating`,
        );
        try {
