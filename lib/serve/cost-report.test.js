...
const OPENAI_ENV = { ...FULL_ENV, CCROTATE_PAPERCLIP_COST_PROVIDER: 'openai', CCROTATE_PAPERCLIP_COST_BILLER: 'ccrotate' };
...
describe('Responses-API (Codex) usage', () => {
  ...
  it('recordFromResponsesJson → flush posts an openai-provider cost-event', async () => {
    const fetchImpl = vi.fn(async () => ({ status: 201, ok: true }));
    const r = createCostReporter({ cfg: costReportConfig(OPENAI_ENV), fetchImpl, nowIso: () => 'T0' });
