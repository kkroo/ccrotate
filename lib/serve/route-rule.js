...
export function listModels({ hasAnthropic, hasOpenai }) {
  ...
  if (hasAnthropic) {
    for (const id of ANTHROPIC_MODEL_ALIASES) {
      data.push({ id, slug: id, display_name: id, object: 'model', owned_by: 'anthropic' });
    }
  }
  if (hasOpenai) {
    for (const id of OPENAI_MODELS) {
      data.push({ id, slug: id, display_name: id, object: 'model', owned_by: 'openai' });
    }
  }
  // OpenAI clients read `data`; recent Codex CLI builds read `models`.
  return { object: 'list', data, models: data };
}
