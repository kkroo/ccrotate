...
export function listModels({ hasAnthropic, hasOpenai }) {
  const data = [];
  if (hasAnthropic) {
    for (const id of ANTHROPIC_MODEL_ALIASES) data.push({ id, slug: id, object: 'model', owned_by: 'anthropic' });
  }
  if (hasOpenai) {
    for (const id of OPENAI_MODELS) data.push({ id, slug: id, object: 'model', owned_by: 'openai' });
  }
