...
export function createRouter(deps) {
  const { callMessages, callChat, callResponses, callEmbeddings, profilesDir, serveToken,
          hasAnthropic, hasOpenai, ccrotate } = deps;
  ...
  async function dispatch(req) {
    ...
    if (pathname === '/v1/messages') {
      ...
      const intercepted = await tryIntercept({
        body, endpoint: 'messages', wantsStream, ccrotate, serveStartedAt,
      });
    ...
    if (pathname === '/v1/chat/completions') {
      const intercepted = await tryIntercept({
        body, endpoint: 'chat', wantsStream: false, ccrotate, serveStartedAt,
      });
      if (intercepted) return intercepted;
      const upstream = pickUpstream(body.model);
      if (upstream === null) {
    ...
    if (pathname === '/v1/responses') {
      const wantsSse = String(getHeader(req, 'accept') || '').includes('text/event-stream') ||
                       body.stream === true;
      const intercepted = await tryIntercept({
        body, endpoint: 'responses', wantsStream: wantsSse, ccrotate, serveStartedAt,
      });
      if (intercepted) return intercepted;
      const upstream = pickUpstream(body.model);
      if (upstream === null) {
    ...
    if (pathname === '/v1/internal/probe-one') {
      ...
      try {
        ...
        const result = await probeOne(body.target, body.email, ccrotate);
