  ...
  async function dispatch(req) {
    ...
    if (pathname === '/v1/internal/pool-status' && req.method === 'GET') {
      try {
        ...
        const tierByEmail = new Map((tierCache.accounts || []).map((a) => [a.email, a]));
    ...
    if (pathname === '/v1/messages') {
      ...
      const resultPromise = callMessages(body, { profilesDir });
      if (wantsStream) {
        return {
          status: 200,
          headers: {
          ...
          stream: messagesStreamWithKeepalive(resultPromise, streamKeepaliveMs),
        };
      }
