...
describe('tryIntercept — dispatch', () => {
  ...
  function makeFakeStore(overrides = {}) {
    return {
      ...
      getRateLimitState: vi.fn(async () => ({ version: 1, updatedAt: null, anthropic: { accounts: {} } })),
  ...
  it('cmd=when hydrates from the store, then renders a pool table', async () => {
    ...
    expect(store.getRateLimitState).toHaveBeenCalled();
...
describe('HANDLERS.setsession (G5 — operator sessionKey paste)', () => {
  ...
  it('falls back to /register when /setSession 400s "email not in claude pool"', async () => {
    process.env.CCROTATE_RELOGIN_TRIGGER_URL = 'http://bot-test:7000';
    const calls = [];
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (url, init) => {
      calls.push({ url: String(url), body: JSON.parse(init.body) });
      if (String(url).endsWith('/setSession')) {
        return {
          ok: false,
          status: 400,
          text: async () => '{"error":"email not in claude pool (14 known)"}',
          json: async () => ({ error: 'email not in claude pool (14 known)' }),
        };
      }
      // /register success
      return {
        ...
        json: async () => ({
          ...
          relogin: { ok: true, snapStdout: '✓ Account bot6@x.com saved successfully.' },
        }),
        text: async () => '',
      };
    });
    try {
      const out = await HANDLERS.setsession({ args: `bot6@x.com ${validSessionKey}` });
      // First call hits /setSession, second falls back to /register
      expect(calls).toHaveLength(2);
      expect(calls[0].url).toBe('http://bot-test:7000/setSession');
      expect(calls[1].url).toBe('http://bot-test:7000/register');
      expect(calls[1].body).toEqual({ email: 'bot6@x.com', target: 'claude', sessionKey: validSessionKey });
      expect(out).toContain('✓ bot6@x.com registered as new pool member');
      expect(out).toContain('Account bot6@x.com saved successfully');
    } finally {
      fetchSpy.mockRestore();
      delete process.env.CCROTATE_RELOGIN_TRIGGER_URL;
    }
  ...
  it('does NOT fall back to /register on non-"not in pool" 400 errors', async () => {
    process.env.CCROTATE_RELOGIN_TRIGGER_URL = 'http://bot-test:7000';
    const calls = [];
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockImplementation(async (url) => {
      ...
      if (String(url).endsWith('/setSession')) {
        return {
          ok: false,
          status: 400,
          text: async () => '{"error":"sessionKey rejected: malformed"}',
          json: async () => ({ error: 'sessionKey rejected: malformed' }),
        };
      }
      throw new Error('should not reach /register for non-pool-member errors');
    });
    try {
      const out = await HANDLERS.setsession({ args: `bot4@x.com ${validSessionKey}` });
      expect(calls).toHaveLength(1);
      expect(calls[0].url).toBe('http://bot-test:7000/setSession');
      expect(out).toContain('returned 400');
      expect(out).toContain('sessionKey rejected');
    } finally {
      fetchSpy.mockRestore();
      delete process.env.CCROTATE_RELOGIN_TRIGGER_URL;
    }
