...
import { callMessages, classifyQuotaError, isOrganizationDisabledMessage, pickNextCandidate } from './anthropic-client.js';
import { resetAnthropicAdmissionForTests } from './anthropic-admission.js';
...
describe('anthropic-client.callMessages — happy path', () => {
  ...
  afterEach(() => { resetAnthropicAdmissionForTests(); vi.restoreAllMocks(); fs.rmSync(dir, { recursive: true, force: true }); });
  ...
  it('emits structured request attribution logs without secrets', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com', 'TOK_SECRET') }, 'a@x.com');
    vi.spyOn(global, 'fetch').mockResolvedValue(
      new Response('{}', {
        status: 200,
        headers: {
          'request-id': 'req_anth_123',
      ...
      {model:'claude-haiku-4-5-20251001', max_tokens:10, messages:[{role:'user', content:'ping'}]},
      {
        ...
        attribution: {
          ...
          caller: {
            ...
            paperclipRunId: 'run_456',
            paperclipAdapterType: 'opencode_k8s',
          },
        },
      }
    );

    const attributionLines = logSpy.mock.calls
      .map(([line]) => String(line))
      .filter((line) => line.startsWith('[callMessages.attribution] '));
    expect(attributionLines.length).toBeGreaterThanOrEqual(3);
    const attemptLine = attributionLines.find((line) => line.includes('"event":"attempt_result"'));
    expect(attemptLine).toContain('"requestId":"ccr_req_123"');
    expect(attemptLine).toContain('"account":"a@x.com"');
    expect(attemptLine).toContain('"status":200');
    expect(attemptLine).toContain('"anthropicRequestId":"req_anth_123"');
    const allLogs = logSpy.mock.calls.map(([line]) => String(line)).join('\n');
    expect(allLogs).not.toContain('TOK_SECRET');
    expect(allLogs).not.toContain('rt-a@x.com');
    expect(allLogs).not.toContain('ping');
  });
  ...
  it('rewrites deprecated top-level output_format to output_config.format', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com') }, 'a@x.com');
    ...
    await callMessages(
      { model: 'claude-opus-4-7', max_tokens: 50, messages: [{role:'user', content:'x'}], output_format: schema },
      { profilesDir: dir },
    );
    const sent = JSON.parse(spy.mock.calls[0][1].body);
    expect(sent.output_format).toBeUndefined();
    expect(sent.output_config).toEqual({ format: schema });
  });

  it('merges rewritten format into an existing output_config (e.g. effort)', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com') }, 'a@x.com');
    const spy = vi.spyOn(global, 'fetch').mockResolvedValue(new Response('{}', { status: 200 }));
    const schema = { type: 'json_schema', schema: { type: 'object' } };
    await callMessages(
      { model: 'claude-opus-4-7', max_tokens: 50, messages: [{role:'user', content:'x'}],
        output_format: schema, output_config: { effort: 'high' } },
      { profilesDir: dir },
    );
    const sent = JSON.parse(spy.mock.calls[0][1].body);
    expect(sent.output_format).toBeUndefined();
    expect(sent.output_config).toEqual({ effort: 'high', format: schema });
  });

  it('does not clobber a client-supplied output_config.format; drops the deprecated field', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com') }, 'a@x.com');
    const spy = vi.spyOn(global, 'fetch').mockResolvedValue(new Response('{}', { status: 200 }));
    const newFmt = { type: 'json_schema', schema: { type: 'object', title: 'new' } };
    const oldFmt = { type: 'json_schema', schema: { type: 'object', title: 'old' } };
    await callMessages(
      { model: 'claude-opus-4-7', max_tokens: 50, messages: [{role:'user', content:'x'}],
        output_format: oldFmt, output_config: { format: newFmt } },
      { profilesDir: dir },
    );
    const sent = JSON.parse(spy.mock.calls[0][1].body);
    expect(sent.output_format).toBeUndefined();
    expect(sent.output_config).toEqual({ format: newFmt });
  });
...
describe('anthropic-client.callMessages — admission throttle', () => {
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); resetAnthropicAdmissionForTests(); });
  afterEach(() => { resetAnthropicAdmissionForTests(); vi.restoreAllMocks(); fs.rmSync(dir, { recursive: true, force: true }); });

  it('waits before an upstream attempt when model-group rpm is capped', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com') }, 'a@x.com');
    vi.spyOn(global, 'fetch').mockResolvedValue(new Response('{}', { status: 200 }));
    const sleepSpy = vi.fn(async () => {});

    await callMessages(
      {model:'claude-opus-4-7', max_tokens:10, messages:[{role:'user', content:'first'}]},
      { profilesDir: dir, admissionConfig: { rpm: 60, inputTpm: 0, outputTpm: 0, maxConcurrency: 0, maxWaitMs: 60_000 }, sleep: sleepSpy }
    );
    await callMessages(
      {model:'claude-opus-4-7', max_tokens:10, messages:[{role:'user', content:'second'}]},
      { profilesDir: dir, admissionConfig: { rpm: 60, inputTpm: 0, outputTpm: 0, maxConcurrency: 0, maxWaitMs: 60_000 }, sleep: sleepSpy }
    );

    expect(sleepSpy).toHaveBeenCalledOnce();
    expect(sleepSpy.mock.calls[0][0]).toBeGreaterThanOrEqual(900);
  });

  it('logs admission_wait attribution when it delays a request', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com') }, 'a@x.com');
    vi.spyOn(global, 'fetch').mockResolvedValue(new Response('{}', { status: 200 }));
    const sleepSpy = vi.fn(async () => {});
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    await callMessages(
      {model:'claude-opus-4-7', max_tokens:10, messages:[{role:'user', content:'first'}]},
      { profilesDir: dir, admissionConfig: { rpm: 60, inputTpm: 0, outputTpm: 0, maxConcurrency: 0, maxWaitMs: 60_000 }, sleep: sleepSpy, attribution: { requestId: 'first', model: 'claude-opus-4-7' } }
    );
    await callMessages(
      {model:'claude-opus-4-7', max_tokens:10, messages:[{role:'user', content:'second'}]},
      { profilesDir: dir, admissionConfig: { rpm: 60, inputTpm: 0, outputTpm: 0, maxConcurrency: 0, maxWaitMs: 60_000 }, sleep: sleepSpy, attribution: { requestId: 'second', model: 'claude-opus-4-7' } }
    );

    const line = logSpy.mock.calls
      .map(([msg]) => String(msg))
      .find((msg) => msg.startsWith('[callMessages.attribution] ') && msg.includes('"event":"admission_wait"'));
    expect(line).toContain('"modelGroup":"claude-opus"');
    expect(line).toContain('"rpm":60');
  });
});
...
describe('anthropic-client.callMessages — empty pool at entry (BLO-8611 null-account guard)', () => {
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); });
  afterEach(() => { resetAnthropicAdmissionForTests(); vi.restoreAllMocks(); fs.rmSync(dir, { recursive: true, force: true }); });

  it('returns a retryable 429 (not a thrown TypeError / 500) when every account lacks a usable token', async () => {
    // Profiles present but none has an OAuth accessToken → pickNextCandidate
    // returns null at entry. Pre-fix, the request_start log dereferenced
    // cand.email and threw "Cannot read properties of null (reading 'email')",
    // which the serve handler mapped to a fatal HTTP 500.
    writeProfiles(dir, {
      'a@x.com': { email: 'a@x.com', credentials: { claudeAiOauth: { refreshToken: 'rt-a' } } },
    }, 'a@x.com');
    ...
    expect(result.trigger).toBe('no-usable-account');
    expect(result.account).toBe(null);
    expect(result.poolExhausted).toBe(true);
    expect(result.attempts).toBe(0);
    // Never reached upstream — there was no account to call with.
    expect(fetchSpy).not.toHaveBeenCalled();
    const body = await result.response.json();
    expect(body.error.type).toBe('rate_limit_error');
  });

  it('also returns 429 when the profile set is completely empty', async () => {
    writeProfiles(dir, {}, null);
    const result = await callMessages(
      { model: 'claude-haiku-4-5-20251001', max_tokens: 10, messages: [{ role: 'user', content: 'ping' }] },
      { profilesDir: dir }
    );
...
describe('isOrganizationDisabledMessage — detect 403 admin-disabled bodies', () => {
  it('matches the canonical Claude Code subscription-disabled message', () => {
    expect(isOrganizationDisabledMessage({
      type: 'error',
      error: {
        type: 'permission_error',
        message: 'Your organization has disabled Claude Code subscription access. Use an Anthropic API key instead, or ask your admin to enable access.',
      },
    })).toBe(true);
  });

  it('matches case-insensitively (Anthropic occasionally re-cases error strings)', () => {
    expect(isOrganizationDisabledMessage({
      error: {
        type: 'PERMISSION_ERROR',
        message: 'YOUR ORGANIZATION HAS DISABLED CLAUDE CODE SUBSCRIPTION ACCESS.',
      },
    })).toBe(true);
  });

  it('matches when only the "Anthropic API key instead" hint is in the message', () => {
    expect(isOrganizationDisabledMessage({
      error: {
        type: 'permission_error',
        message: 'Use an Anthropic API key instead.',
      },
    })).toBe(true);
  });

  it('returns false for permission_error bodies without subscription-disabled signals', () => {
    expect(isOrganizationDisabledMessage({
      error: { type: 'permission_error', message: 'Forbidden.' },
    })).toBe(false);
  });

  it('returns false for non-permission_error bodies even with the magic strings', () => {
    // Defensive: a different error.type with the same words should not trip
    // a stale-mark — Anthropic's permission_error is the authoritative signal.
    expect(isOrganizationDisabledMessage({
      error: { type: 'rate_limit_error', message: 'Use an Anthropic API key instead.' },
    })).toBe(false);
  });

  it('returns false for null / empty bodies', () => {
    expect(isOrganizationDisabledMessage(null)).toBe(false);
    expect(isOrganizationDisabledMessage({})).toBe(false);
    expect(isOrganizationDisabledMessage({ error: null })).toBe(false);
  });
});

describe('anthropic-client.callMessages — 403 organization-disabled stale-mark', () => {
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); });
  afterEach(() => { resetAnthropicAdmissionForTests(); vi.restoreAllMocks(); fs.rmSync(dir, { recursive: true, force: true }); });

  it('marks the account stale on 403 + permission_error, rotates to next, returns next 200', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    const fetchSpy = vi.spyOn(global, 'fetch').mockImplementation((url, opts) => {
      ...
      // b@x.com — happy path
      return Promise.resolve(new Response(
        JSON.stringify({ id: 'msg_B', type: 'message', role: 'assistant',
    ...
    expect(result.account).toBe('b@x.com');
    expect(result.attempts).toBe(2);
    expect(result.trigger).toBe('organization-disabled');

    // a@x.com is now flagged stale with reason + truncated response.
    const profiles = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
    expect(profiles['a@x.com'].stale).toBe(true);
    expect(profiles['a@x.com'].staleReason).toBe('organization_disabled');
    expect(profiles['a@x.com'].staleResponse).toContain('subscription access');
    expect(typeof profiles['a@x.com'].staleAt).toBe('string');
    // Access token preserved (admin re-enable is the recovery path).
    expect(profiles['a@x.com'].credentials.claudeAiOauth.accessToken).toBe('TOK_A');
    // b@x.com is untouched.
    expect(profiles['b@x.com'].stale).not.toBe(true);
  });

  it('returns the 403 verbatim when no other account is available (pool walked)', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com', 'TOK_A') }, 'a@x.com');
    ...
    // After marking a@ stale + walking, no candidate is left — the caller
    // gets the last response (403) so it sees the upstream error rather
    // than a silent 502.
  ...
  it('does not mark stale on a non-matching 403 (e.g. invalid permission scope)', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    const profiles = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
    // Neither profile is flagged — the 403 falls through to the success-
    // return path (caller decides how to handle a generic permission_error).
    expect(profiles['a@x.com'].stale).not.toBe(true);
    expect(profiles['b@x.com'].stale).not.toBe(true);
  });
});

describe('pickNextCandidate — skips stale profiles', () => {
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); });
  afterEach(() => { fs.rmSync(dir, { recursive: true, force: true }); });

  it('returns the next non-stale candidate when the active profile is stale', async () => {
    writeProfiles(dir, {
      'a@x.com': { ...profile('a@x.com'), stale: true, staleAt: new Date().toISOString() },
      'b@x.com': profile('b@x.com'),
    }, 'a@x.com');
    const store = new FileStateStore(dir);
    const cand = await pickNextCandidate(store, new Set(), 'claude-haiku-4-5-20251001');
    expect(cand?.email).toBe('b@x.com');
  });

  it('returns null when every profile is stale', async () => {
    writeProfiles(dir, {
      'a@x.com': { ...profile('a@x.com'), stale: true },
      'b@x.com': { ...profile('b@x.com'), stale: true },
    }, 'a@x.com');
    const store = new FileStateStore(dir);
    const cand = await pickNextCandidate(store, new Set(), 'claude-haiku-4-5-20251001');
...
describe('anthropic-client.callMessages — transient 529 overload wait', () => {
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); });
  afterEach(() => { vi.restoreAllMocks(); fs.rmSync(dir, { recursive: true, force: true }); });
  ...
  it('waits and retries the same account on Anthropic 529 instead of returning it to Claude Code', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    const result = await callMessages(ping, {
      profilesDir: dir,
      sleep: sleepSpy,
      transient529BaseDelayMs: 100,
      transient529MaxDelayMs: 500,
      transient529MaxRetries: 5,
      transient529WaitBudgetMs: 2_000,
    });
    expect(result.status).toBe(200);
    expect(result.account).toBe('a@x.com');
    expect(result.trigger).toBe('transient-529');
    expect(sleepSpy.mock.calls.map(([ms]) => ms)).toEqual([100, 200]);
  });

  it('returns 529 after the transient overload retry budget is exhausted', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com', 'TOK_A') }, 'a@x.com');
    vi.spyOn(global, 'fetch').mockResolvedValue(
      new Response(
        JSON.stringify({ error: { type: 'overloaded_error', message: 'Overloaded' } }),
    ...
    const result = await callMessages(ping, {
      ...
      sleep: sleepSpy,
      transient529BaseDelayMs: 100,
      transient529MaxDelayMs: 100,
      transient529MaxRetries: 2,
      transient529WaitBudgetMs: 1_000,
    });
...
describe('anthropic-client.callMessages — wait-on-same-account for transient 429', () => {
  // Issue (2026-05-21, kkroo#63 follow-up). Rotating on transient 429 chains
  // into a refresh attempt on the next account's stale access token. That
  // refresh can itself 429 the per-org /oauth/token endpoint and (pre-#63)
  // fire triggerRelogin → auth-bot Camoufox cycle → fresh refresh_token
  // written over a still-valid one. Even post-#63, rotation still burns a
  // different account's quota for what is typically a 1-5s burst-throttle.
  // For sub-`waitOnTransient429MaxSec` retry-afters, wait on the same account
  // and short-circuit the whole chain.
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); });
  afterEach(() => { vi.restoreAllMocks(); try { fs.rmSync(dir, { recursive: true, force: true }); } catch {} });
  ...
  it('waits on the SAME account when retry-after is small, no rotation', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, opts) => {
      call += 1;
      if (call === 1) {
        // First call to a@: transient 429 with short retry-after
        expect(opts.headers?.Authorization).toBe('Bearer TOK_A');
        return new Response(
          JSON.stringify({ error: { type: 'rate_limit_error', message: 'burst' } }),
          { status: 429, headers: { 'retry-after': '3' } },
        );
      }
      // Retry on a@ succeeds (no rotation off it)
      expect(opts.headers?.Authorization).toBe('Bearer TOK_A');
      return new Response(okBody, { status: 200 });
    });
    const sleepSpy = vi.fn(async () => {});
    const result = await callMessages(ping, {
      profilesDir: dir, sleep: sleepSpy,
      waitOnTransient429MaxSec: 60, transient429BackoffMs: 0,
    });
    expect(result.status).toBe(200);
    expect(result.account).toBe('a@x.com');  // never rotated
    expect(sleepSpy).toHaveBeenCalledTimes(1);
    // (retry-after + 1) * 1000 → 4000ms
    expect(sleepSpy.mock.calls[0][0]).toBe(4000);
  });

  it('rotates instead of waiting when retry-after exceeds the ceiling', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, opts) => {
      ...
      if (call === 1) {
        return new Response(
          JSON.stringify({ error: { type: 'rate_limit_error', message: 'short burst' } }),
          // 60s is not < ceiling of 60s. classifyQuotaError demotes (horizon < 600s) → transient.
    ...
    expect(result.status).toBe(200);
    expect(result.account).toBe('b@x.com');  // rotated
    // No wait, no backoff (transient429BackoffMs=0)
    expect(sleepSpy).not.toHaveBeenCalled();
  });

  it('waits repeatedly on the same account until a transient 429 clears', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, opts) => {
      call += 1;
      if (call === 1) {
        // First a@: transient 429, waitable
        return new Response(
      ...
      if (call === 2) {
        // Retry on a@ STILL transient 429 → wait again, still no rotation.
        expect(opts.headers?.Authorization).toBe('Bearer TOK_A');
        return new Response(
          JSON.stringify({ error: { type: 'rate_limit_error' } }),
          { status: 429, headers: { 'retry-after': '2' } },
    ...
    expect(result.account).toBe('a@x.com');
    expect(sleepSpy).toHaveBeenCalledTimes(2);
    expect(sleepSpy.mock.calls[0][0]).toBe(3000);
    expect(sleepSpy.mock.calls[1][0]).toBe(3000);
  });

  it('rotates after same-account transient retry budget is exhausted', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    const result = await callMessages(ping, {
      profilesDir: dir, sleep: sleepSpy,
      waitOnTransient429MaxSec: 60,
      transient429SameAccountMaxRetries: 2,
      transient429BackoffMs: 0,
    });
    expect(result.status).toBe(200);
    expect(result.account).toBe('b@x.com');
    expect(sleepSpy).toHaveBeenCalledTimes(2);
  ...
  it('waitOnTransient429MaxSec=0 disables wait — falls through to rotate', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    const result = await callMessages(ping, {
      profilesDir: dir, sleep: sleepSpy,
      waitOnTransient429MaxSec: 0, transient429BackoffMs: 0,
    });
    expect(result.status).toBe(200);
    expect(result.account).toBe('b@x.com');  // rotated
    expect(sleepSpy).not.toHaveBeenCalled();
  ...
  it('no retry-after header → rotates (no signal to wait on)', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
...
describe('anthropic-client.callMessages — verify-on-429 via Usage API', () => {
  ...
  it('demotes structural 429 to transient when probe returned STALE LKG, regardless of util%', async () => {
    // Stale-LKG fix (2026-05-20 false-exhausted pool incident). Pre-fix,
    // a stale LKG probe showing util7d=100% would CONFIRM structural and
    // bake a multi-day exhaustion flag into tier-cache, even though the
    // LKG value was hours old and could no longer reflect reality.
    // Observed live: a seat at 79% actual weekly capacity got flagged
    // exhausted until 2026-05-23 because every 429 verified against the
    // same stale peak-util LKG.
    //
    // Post-fix: stale LKG is "no confirmation". Rotate as transient,
    // don't poison tier-cache. Only fresh probes confirm structural.
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    // High-util but stale=true — pre-fix would have confirmed structural;
    // post-fix must demote to transient.
    const probe = vi.fn(async () => ({ utilization5h: 100, utilization7d: 100, stale: true }));
    ...
    expect(result.account).toBe('b@x.com');
    expect(result.trigger).toBe('transient-429');
    // tier-cache must NOT have been written — no false-exhausted flag.
    expect(fs.existsSync(path.join(dir, 'tier-cache.json'))).toBe(false);
  });
  ...
  it('does not probe when the 429 was classified transient (no structural escalation)', async () => {
    ...
    const result = await callMessages(ping, { profilesDir: dir, probeUsageApi: probe, sleep: sleepSpy });

    expect(probe).not.toHaveBeenCalled();
    expect(result.trigger).toBe('transient-429');
    expect(result.account).toBe('a@x.com');
    expect(sleepSpy).toHaveBeenCalledWith(6000);
  });
});
...
describe('anthropic-client.callMessages — cross-write self-heal on 429-structural', () => {
  ...
  it('rechecks and quarantines when the offending token rotated to another wrong token before quarantine landed', async () => {
    writeProfiles(dir, {
      'wrong-key@x.com': profile('wrong-key@x.com', 'TOK_OLD'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'wrong-key@x.com');
    let call = 0;
    vi.spyOn(global, 'fetch').mockImplementation(async () => {
    ...
    const probeUsage = vi.fn(async () => ({ utilization5h: 100, utilization7d: 99, stale: false }));
    const probeProfile = vi.fn(async (tok) => {
      if (tok === 'TOK_OLD') return { email: 'real-owner@x.com', uuid: 'u-real', stale: false };
      if (tok === 'TOK_NEW_WRONG') return { email: 'real-owner@x.com', uuid: 'u-real', stale: false };
      return null;
    });
    const store = new FileStateStore(dir);
    const realMarkCrossWritten = store.markCrossWritten.bind(store);
    const markSpy = vi.spyOn(store, 'markCrossWritten').mockImplementation(async (email, detectedIdentity, offendingAccessToken, ...rest) => {
      if (offendingAccessToken === 'TOK_OLD') {
        const file = path.join(dir, 'profiles.json');
        const profiles = JSON.parse(fs.readFileSync(file, 'utf8'));
        profiles[email].credentials.claudeAiOauth.accessToken = 'TOK_NEW_WRONG';
        profiles[email].credentials.claudeAiOauth.refreshToken = 'RT_NEW_WRONG';
        fs.writeFileSync(file, JSON.stringify(profiles));
      }
      return realMarkCrossWritten(email, detectedIdentity, offendingAccessToken, ...rest);
    });
    const trigger = vi.fn(() => true);
    ...
    expect(result.status).toBe(200);
    expect(markSpy).toHaveBeenCalledWith('wrong-key@x.com', 'real-owner@x.com', 'TOK_OLD');
    expect(markSpy).toHaveBeenCalledWith('wrong-key@x.com', 'real-owner@x.com', 'TOK_NEW_WRONG');
    const after = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
    expect(after['wrong-key@x.com'].credentials.claudeAiOauth.accessToken).toBe(null);
    expect(after['wrong-key@x.com'].credentials.claudeAiOauth.refreshToken).toBe(null);
    expect(after['wrong-key@x.com'].stale).toBe(true);
    expect(after['wrong-key@x.com'].crossWriteIdentity).toBe('real-owner@x.com');
    expect(trigger).toHaveBeenCalledWith('wrong-key@x.com', 'claude');
  });
...
describe('anthropic-client.callMessages — B8 cross-write self-heal on 401-refresh write side', () => {
  // Paperclip incident 2026-05-20 follow-up. The 401→refresh path was
  // perpetuating cross-writes: if profiles.json had a refreshToken
  // cross-wired (held bot1's RT under bot2's profile key), the refresh
  // request would mint fresh bot1 tokens, and the old writeProfileToken
  // would write them back under bot2's email key. The cycle repeated
  // every refresh tick, with G1 (read-side) catching + quarantining
  // each time and stale-poller relogging — only to be cross-wired again
  // on the next refresh. PR #46/#50 patched the READ side; B8 mirrors
  // that on the WRITE side: probe the freshly-minted accessToken's
  // identity BEFORE writing, refuse on mismatch, quarantine, and fire
  // relogin.
  let dir;
  beforeEach(() => { dir = tmpProfilesDir(); });
  afterEach(() => { vi.restoreAllMocks(); fs.rmSync(dir, { recursive: true, force: true }); });
  ...
  it('refuses writeProfileToken + quarantines + fires relogin when refresh mints wrong-identity tokens', async () => {
    writeProfiles(dir, {
      'cross-wired@x.com': profile('cross-wired@x.com', 'TOK_OLD'),
      'fallback@x.com': profile('fallback@x.com', 'TOK_FB'),
    }, 'cross-wired@x.com');
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, opts) => {
      call += 1;
      if (call === 1) {
        // First Messages call: 401 (refresh needed)
        expect(opts.headers['Authorization']).toBe('Bearer TOK_OLD');
        return new Response('{"error":{"type":"authentication_error"}}', { status: 401 });
      }
      if (call === 2) {
        // Token refresh: succeeds, mints fresh tokens — but for the WRONG identity
        expect(url).toContain('/oauth/token');
        return new Response(JSON.stringify({
          access_token: 'TOK_NEW_WRONG_IDENTITY',
          refresh_token: 'RT_NEW_WRONG_IDENTITY',
          expires_in: 3600,
        }), { status: 200 });
      }
      // 3rd: replay on fallback account
      expect(opts.headers['Authorization']).toBe('Bearer TOK_FB');
      return new Response(okBody, { status: 200 });
    });

    // Identity probe says the freshly-minted access token belongs to a
    // DIFFERENT identity than the profile we just refreshed for.
    const probeProfile = vi.fn(async (tok) => {
      if (tok === 'TOK_NEW_WRONG_IDENTITY') {
        return { email: 'real-owner@x.com', uuid: 'u-real', stale: false };
      }
      return null;
    });
    const trigger = vi.fn(() => true);
    ...
    // The original request still got served — by the fallback account.
    expect(result.status).toBe(200);
    expect(result.account).toBe('fallback@x.com');
    expect(result.trigger).toBe('refresh-fail');

    // profiles.json was NOT clobbered with the wrong-identity token.
    // Instead, the cross-wired profile got nulled + flagged stale by
    // markCrossWritten (same handling as the read-side).
    const after = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
    expect(after['cross-wired@x.com'].credentials.claudeAiOauth.accessToken).toBe(null);
    expect(after['cross-wired@x.com'].credentials.claudeAiOauth.refreshToken).toBe(null);
    expect(after['cross-wired@x.com'].stale).toBe(true);
    expect(after['cross-wired@x.com'].crossWriteIdentity).toBe('real-owner@x.com');

    // Quarantine file captures BOTH the stale on-disk pair AND the
    // freshly-minted offending pair so an operator can forensically
    // reconstruct the refresh-write attempt.
    const q = JSON.parse(fs.readFileSync(path.join(dir, 'cross-write-quarantine.json'), 'utf8'));
    expect(q['cross-wired@x.com'].history).toHaveLength(1);
    const entry = q['cross-wired@x.com'].history[0];
    expect(entry.detectedIdentity).toBe('real-owner@x.com');
    expect(entry.oauthSnapshot.accessToken).toBe('TOK_OLD');
    expect(entry.oauthSnapshot.refreshToken).toBe('rt-cross-wired@x.com');
    expect(entry.offendingOauthSnapshot.accessToken).toBe('TOK_NEW_WRONG_IDENTITY');
    expect(entry.offendingOauthSnapshot.refreshToken).toBe('RT_NEW_WRONG_IDENTITY');
    expect(entry.offendingOauthSnapshot.source).toBe('refresh-write-refused');

    // Relogin was fired for the cross-wired account (auth-bot will heal it).
    expect(trigger).toHaveBeenCalledWith('cross-wired@x.com', 'claude');
  });

  it('writes the new token normally when refresh mints SAME-identity tokens (happy path unchanged)', async () => {
    writeProfiles(dir, { 'a@x.com': profile('a@x.com', 'TOK_OLD') }, 'a@x.com');
    ...
    vi.spyOn(global, 'fetch').mockImplementation(async (url, opts) => {
      ...
      if (call === 2) {
        expect(url).toContain('/oauth/token');
        return new Response(JSON.stringify({
          access_token: 'TOK_NEW', refresh_token: 'RT_NEW', expires_in: 3600,
        }), { status: 200 });
      }
      // Replay
      expect(opts.headers['Authorization']).toBe('Bearer TOK_NEW');
    ...
    // Probe matches → no cross-write
    const probeProfile = vi.fn(async () => ({ email: 'a@x.com', uuid: 'u', stale: false }));
    const trigger = vi.fn(() => true);
    ...
    expect(result.status).toBe(200);
    expect(result.account).toBe('a@x.com');
    // Normal write happened
    const after = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
    expect(after['a@x.com'].credentials.claudeAiOauth.accessToken).toBe('TOK_NEW');
    expect(after['a@x.com'].stale).toBeUndefined();
    expect(trigger).not.toHaveBeenCalled();
    // No quarantine file written
    expect(fs.existsSync(path.join(dir, 'cross-write-quarantine.json'))).toBe(false);
  });

  it('writes normally when identity probe returns null (no signal, conservative)', async () => {
    // Probe failure or cooldown should fall back to pre-B8 behavior:
    // assume the refresh is fine, write the token, don't spuriously
    // quarantine. Errors-as-no-signal mirrors the read-side default.
    writeProfiles(dir, { 'a@x.com': profile('a@x.com', 'TOK_OLD') }, 'a@x.com');
    ...
    const probeProfile = vi.fn(async () => null); // probe couldn't decide
    const trigger = vi.fn(() => true);
  ...
  it('swallows a thrown identity probe during refresh-write without aborting (still writes)', async () => {
    // If the probe itself throws, we can't decide — choose the conservative
    // path: write the token and continue. Don't poison the pool on a
    // network blip. Same shape as the read-side swallow test.
    writeProfiles(dir, { 'a@x.com': profile('a@x.com', 'TOK_OLD') }, 'a@x.com');
    ...
    const probeProfile = vi.fn(async () => { throw new Error('probe boom'); });
    const trigger = vi.fn(() => true);
  ...
  it('case-insensitive identity match (operator-entered Mixed-Case email vs Anthropic lowercase)', async () => {
    writeProfiles(dir, { 'Mixed@X.com': profile('Mixed@X.com', 'TOK_OLD') }, 'Mixed@X.com');
    vi.spyOn(global, 'fetch')
      .mockResolvedValueOnce(new Response('{}', { status: 401 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        access_token: 'TOK_NEW', refresh_token: 'RT_NEW', expires_in: 3600,
    ...
    // Anthropic returns lowercase email — should match Mixed-Case profile key
    const probeProfile = vi.fn(async () => ({ email: 'mixed@x.com', uuid: 'u', stale: false }));
    const trigger = vi.fn(() => true);
    ...
    // No cross-write — normal write happened
    expect(trigger).not.toHaveBeenCalled();
    const after = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
    expect(after['Mixed@X.com'].credentials.claudeAiOauth.accessToken).toBe('TOK_NEW');
  });
});
...
describe('anthropic-client.callMessages — auto-relogin trigger on refresh-fail', () => {
  ...
  // Transient refresh-failure regression (2026-05-21). When /api/oauth/token/refresh
  // returns 429 (per-org refresh-endpoint rate limit, common when a burst of
  // 401-on-expiry causes concurrent refreshes), the stored refresh_token is
  // still valid — the refresh endpoint just declined to honor THIS attempt
  // right now. The pre-fix code swallowed the throw without inspecting the
  // status and unconditionally fired triggerRelogin, driving the auth-bot
  // through a full Camoufox-based OAuth cycle, overwriting a still-valid
  // refresh_token with a fresh one and churning the pool. Symptom: pool's
  // 5h utilization saturates fast because rotation hops accounts on every
  // transient burst instead of staying on one until it genuinely caps.
  // The fix preserves the relogin trigger for PERMANENT failures (4xx
  // excluding 429 — refresh token actually revoked) and rotates without
  // a relogin for TRANSIENT failures (429, 5xx, network error).

  it('does NOT fire triggerRelogin when refresh returns 429 (transient)', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    ...
    expect(result.status).toBe(200);
    expect(result.account).toBe('b@x.com');
    expect(result.trigger).toBe('transient-refresh-error');
    // refresh_token is still valid — auth-bot relogin is NOT needed.
    expect(trigger).not.toHaveBeenCalled();
  });

  it('does NOT fire triggerRelogin when refresh returns 503 (transient 5xx)', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
  ...
  it('does NOT fire triggerRelogin when the refresh call itself throws (network error)', async () => {
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
    vi.spyOn(global, 'fetch').mockImplementation(async (url, opts) => {
      if (opts.headers?.['Authorization'] === 'Bearer TOK_A')
        return new Response('{}', { status: 401 });
      if (url === 'https://api.anthropic.com/api/oauth/token/refresh') {
        // Simulate a fetch-level network error (DNS, timeout, etc).
  ...
  it('still fires triggerRelogin when refresh returns 400 (permanent, refresh_token dead)', async () => {
    // Counterpart to the 429/503/network tests above — make sure the
    // narrowed condition didn't silently disable the self-heal trigger
    // for the genuine refresh-token-revoked case.
    writeProfiles(dir, {
      'a@x.com': profile('a@x.com', 'TOK_A'),
      'b@x.com': profile('b@x.com', 'TOK_B'),
    }, 'a@x.com');
...
describe('pickNextCandidate — active pointer respects exhaustion', () => {
  ...
  it('skips the active email when tier-cache marks it exhausted', async () => {
    const store = mkStore({
      profiles: {
  ...
  it('returns null when the active email is exhausted and no other usable account exists', async () => {
    const store = mkStore({
      profiles: {
        'bot5@blockcast.net': mkProfile('bot5@blockcast.net'),
      },
