...
describe('classifyQuotaError — burst-429 vs real exhaustion', () => {
  ...
  it('converts retry-after duration-in-seconds to an absolute reset epoch (not raw value)', () => {
    // Regression: `retry-after` is an HTTP-spec duration in seconds, NOT a
    // unix epoch. Pre-fix the code stored the raw number as `reset5h`, so a
    // retry-after of e.g. 60 became reset5h=60 → tier-cache showed
    // "quota exhausted; resets at 1970-01-01T00:01:00.000Z" and
    // pickNextCandidate's `reset5h * 1000 <= Date.now()` filter treated
    // the account as ALWAYS expired (1970 epochs are in the distant past).
    // Fix: when falling back to retry-after, add Date.now()/1000 to get a
    // future absolute epoch.
    ...
    expect(result.structural).toBe(true);
    // reset5h should be ~now + 60s, NOT the raw 60.
    expect(result.reset5h).toBeGreaterThanOrEqual(before + 60);
    expect(result.reset5h).toBeLessThanOrEqual(after + 60);
  });
  ...
  it('handles retry-after value of 0 (immediate retry hint) by computing reset = now', () => {
    // Edge case: retry-after: 0 means "retry now" — should still produce
    // a non-1970 epoch (now), not raw 0.
    const body = { error: { type: 'rate_limit_error', message: 'rate limit' } };
    const before = Math.floor(Date.now() / 1000);
    ...
    const after = Math.floor(Date.now() / 1000);
    expect(result.structural).toBe(true);
    expect(result.reset5h).toBeGreaterThanOrEqual(before);
    expect(result.reset5h).toBeLessThanOrEqual(after);
    // Specifically: NOT 1970-era
    expect(result.reset5h).toBeGreaterThan(1_000_000_000);
  });
});
