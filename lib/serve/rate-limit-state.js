...
// Codex/OpenAI families grouped the way ccrotate-serve cares about rate limits:
// /v1/responses tier limits are scoped per model family (gpt-5 vs gpt-4 etc),
// so per-family RPM/TPM lets one slow family throttle without starving others.
export function normalizeCodexModelGroup(model) {
  const m = String(model || '').toLowerCase();
  if (!m) return 'unknown';
  if (m.startsWith('gpt-5') || m.includes('codex-gpt-5')) return 'codex-gpt-5';
  if (m.startsWith('gpt-4') || m.includes('codex-gpt-4')) return 'codex-gpt-4';
  if (m.startsWith('o3')) return 'codex-o3';
  if (m.startsWith('o4')) return 'codex-o4';
  if (m.startsWith('o1')) return 'codex-o1';
  if (m.startsWith('gpt-') || m.includes('codex')) return 'codex-other';
  return 'unknown';
}
...
function hasRateLimitBucketEvidence(buckets) {
  return !!(buckets.requests || buckets.inputTokens || buckets.outputTokens);
}
...
export function applyAnthropicRateLimitHeaders(state, {
  ...
  const currentBuckets = {
    requests: bucketFromHeaders(headers, 'requests', nowIso),
    inputTokens: bucketFromHeaders(headers, 'input-tokens', nowIso),
    outputTokens: bucketFromHeaders(headers, 'output-tokens', nowIso),
  };
  const next = {
    ...existing,
    modelGroup,
    requests: currentBuckets.requests ?? existing.requests,
    inputTokens: currentBuckets.inputTokens ?? existing.inputTokens,
    outputTokens: currentBuckets.outputTokens ?? existing.outputTokens,
    learnedFromHeadersAt: nowIso,
  };

  if (status === 429) {
    const hasCurrentBucketEvidence = hasRateLimitBucketEvidence(currentBuckets);
    if (hasCurrentBucketEvidence) {
      next.cooldownUntil = parseRetryAfter(headers)
        ?? currentBuckets.requests?.resetAt
        ?? currentBuckets.inputTokens?.resetAt
        ?? currentBuckets.outputTokens?.resetAt
        ?? new Date(now.getTime() + 60_000).toISOString();
    } else if (!hasRateLimitBucketEvidence(next)) {
      // A bare 429 + retry-after without Anthropic rate-limit bucket headers
      // is a transient/proxy-level throttle, not proof that this account/model
      // is quota-blocked. Do not publish it as a cross-request pool cooldown.
      delete next.cooldownUntil;
    }
    next.last429At = nowIso;
    next.last429Reason = infer429Reason(currentBuckets);
...
export function clearAnthropicRateLimitState(state, {
  email,
  model = null,
  modelGroup = null,
  now = new Date(),
  ...
  const group = modelGroup || (model ? normalizeAnthropicModelGroup(model) : null);
  if (group) {
    if (!account.modelGroups?.[group]) return nextState;
    const remaining = { ...(account.modelGroups || {}) };
    delete remaining[group];
    if (Object.keys(remaining).length > 0) {
      nextState.anthropic.accounts[email] = { ...account, modelGroups: remaining };
    } else {
      delete nextState.anthropic.accounts[email];
    }
  } else {
    delete nextState.anthropic.accounts[email];
  }
