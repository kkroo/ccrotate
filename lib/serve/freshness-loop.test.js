...
describe('probeOne helper', () => {
  ...
  // probeCodexAccount (the same primitive `refresh-one --target codex`
  // uses) and persists via upsertTierCacheEntries.
  describe('codex probe wiring (probeOne uses probeCodexAccount for target=codex)', () => {
    it("calls probeCodexAccount, NOT testAccount, when target='codex'", async () => {
      const store = fakeStore({
      ...
      const ccrotate = {
        testAccount: vi.fn(),
        probeCodexAccount: vi.fn(() => probeReturn),
      ...
      expect(ccrotate.testAccount).not.toHaveBeenCalled();
      expect(ccrotate.probeCodexAccount).toHaveBeenCalledWith(
        'codex-only@x.com',
        ...
        expect.any(Object), // full profiles map (third arg)
      );
    ...
    it("persists error results via upsertTierCacheEntries too (so codex pool reflects failures)", async () => {
      // probeCodexAccount returns { status: 'error', stale: true } for
      // expired id_tokens (line 1593 of ccrotate.js). The freshness
      // entry needs to land in tier-cache so a future call routing
      // decision sees the error, not a stale healthy snapshot. Matches
      // refresh-one --target codex semantics (writes both success and
      // error results unconditionally).
      const store = fakeStore({
        codexProfiles: {
          'expired@x.com': {
            provider: 'codex',
      ...
      const ccrotate = {
        probeCodexAccount: vi.fn(() => errorReturn),
        upsertTierCacheEntries: vi.fn(),
      };
    ...
    it("emails present in BOTH pools use the codex profile when target='codex'", async () => {
      ...
      // probeCodexAccount; passing the claude shape would crash at
      ...
      const ccrotate = {
        probeCodexAccount: vi.fn(() => ({ status: 'success', serviceTier: 'available', rateLimits: {} })),
        upsertTierCacheEntries: vi.fn(),
      };
    ...
    it("returns error when codex profile has no auth (no probeCodexAccount call)", async () => {
      const store = fakeStore({
    ...
    it("returns an error result on probeCodexAccount throw and writes no entry", async () => {
      const store = fakeStore({
        codexProfiles: {
      ...
      const ccrotate = {
        probeCodexAccount: vi.fn(() => { throw new Error('boom'); }),
        upsertTierCacheEntries: vi.fn(),
      };
