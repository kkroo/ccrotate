...
describe('startFreshnessLoop', () => {
  ...
  it('respects opts.rotationOffset so independent loops start at different candidates', async () => {
    // Three stale-exhausted accounts, all eligible. pickStaleEntry sorts
    // by capturedAt asc. With offset 0 we expect candidates[0]; with offset
    // 1 we expect candidates[1]. This validates the HA-friendly contract
    // where two ccrotate-serve replicas should not collide on tick 0.
    const accounts = [
