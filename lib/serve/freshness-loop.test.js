...
describe('probeOne helper', () => {
  ...
  // Regression coverage for the codex-store wiring. PR #65 fixed the
  // per-target *token* path inside a profile but left the per-target
  // *store* path hardcoded to getProfiles() — so emails that only live
  // in profiles.codex.json (the dedicated codex accounts:
  // omar.ramadan@, princeomz2004@, omar.ramadan93@) always returned
  // "no profile or token for email" because the lookup hit the claude
  // store. Empirically observed against a live cluster on 2026-05-22:
  // all 5 codex pool entries returned that error from
  // POST /v1/internal/probe-one, even ones whose codex profile was
  // present and freshly written by /relogin.
  describe('codex store wiring (probeOne reads profiles.codex.json)', () => {
    it("reads from getCodexProfiles when target='codex'", async () => {
      const store = fakeStore({
      ...
      expect(ccrotate.testAccount).toHaveBeenCalledWith(
        'codex-only@x.com',
        expect.objectContaining({ token: 'CODEX_AT', usageApiOnly: false }),
      );
      expect(result.email).toBe('codex-only@x.com');
      expect(result.serviceTier).toBe('base');
    });

    it("does NOT find codex emails via the claude store (regression guard)", async () => {
      // The exact failure mode this PR is fixing: profile lives ONLY in
      // profiles.codex.json. If probeOne still called getProfiles(), the
      // email would be missing from the lookup and bail with the canned
      // error — even though the profile (and live token) exist.
      const store = fakeStore({
        profiles: { /* empty */ },
        codexProfiles: {
          'codex-only@x.com': {
      ...
      // The fix means we DO find the profile and DO invoke testAccount.
      expect(result.status).not.toBe('error');
      expect(ccrotate.testAccount).toHaveBeenCalledOnce();
    });

    it("emails present in BOTH pools resolve via the target's store", async () => {
      // ally@ and ssh-users+1@ exist in both profiles.json (claude shape)
      // and profiles.codex.json (codex shape). A codex probe must read
      // the codex-shaped profile (which has auth.tokens.access_token);
      // reading the claude shape would fail the token lookup at line 152.
      const store = fakeStore({
        profiles: {
          'ally@x.com': {
      ...
      // Used the codex token, NOT the claude one — proves the codex
      // store was consulted, not the claude store.
      expect(ccrotate.testAccount).toHaveBeenCalledWith(
        'ally@x.com',
        expect.objectContaining({ token: 'CODEX_AT' }),
      );
    });

    it("claude target still reads from getProfiles (regression guard for #65 happy path)", async () => {
      const store = fakeStore({
        profiles: {
          'claude-only@x.com': {
            credentials: { claudeAiOauth: { accessToken: 'CLAUDE_AT', expiresAt: Date.now() + 9e9 } },
