...
describe('router — bearer auth + dispatch', () => {
  ...
  it('wraps /v1/messages stream:true responses as Anthropic SSE', async () => {
    ...
    expect(r.body).toContain('event: content_block_delta');
    expect(r.body).toContain('event: message_stop');
    expect(mocks.callMessages.mock.calls[0][0].stream).toBe(false);
  });

  it('wraps /v1/messages tool_use blocks as Anthropic SSE', async () => {
    mocks.callMessages.mockResolvedValue({
      status: 200,
      response: new Response(JSON.stringify({
        id: 'msg_1',
    ...
    expect(r.status).toBe(200);
    expect(r.body).toContain('"type":"tool_use"');
    expect(r.body).toContain('"type":"input_json_delta"');
    expect(r.body).toContain('"partial_json":"{\\"command\\":\\"printf ok\\"}"');
    expect(r.body).toContain('"stop_reason":"tool_use"');
  });
