...
describe('router — bearer auth + dispatch', () => {
  ...
  it('routes /v1/responses gpt-* → callResponses passthrough', async () => {
    mocks.callResponses.mockResolvedValue({
      status: 200,
      response: new Response(JSON.stringify({id:'resp_x', output: []}), {status: 200}),
      attempts: 1,
    });
    const router = createRouter(mocks);
    const r = await dispatch(router, makeReq({
    ...
    expect(r.status).toBe(200);
    expect(JSON.parse(r.body).id).toBe('resp_x');
  });
