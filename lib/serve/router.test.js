...
describe('router — bearer auth + dispatch', () => {
  ...
  it('returns pool status when tier-cache has null or malformed account rows', async () => {
    mocks.store = {
      getProfiles: vi.fn(async () => ({
        'a@x.net': {},
        'b@x.net': {},
      })),
      getTierCache: vi.fn(async () => ({
        accounts: [
          null,
          {},
          { email: 7, serviceTier: 'base' },
          { email: 'a@x.net', serviceTier: 'base' },
        ],
      })),
      getRateLimitState: vi.fn(async () => ({
        anthropic: { accounts: {} },
      })),
      getActiveEmail: vi.fn(async () => 'a@x.net'),
    };
    const router = createRouter(mocks);
    const r = await dispatch(router, makeReq({
      method: 'GET',
      url: '/v1/internal/pool-status',
      headers: { authorization: 'Bearer tok-secret-32' },
    }));
    ...
    expect(body.accounts).toEqual([
      { email: 'a@x.net', active: true, tier: 'base', rateLimits: {} },
      { email: 'b@x.net', active: false, tier: null, rateLimits: {} },
    ]);
  });
