...
// Optional ccrotate-auth-bot HTTP base URL. When set, the `when`/`accounts`
// interceptors append an "Active identity leases" section so operators can
// see which identities are currently being held by a designer-bot (figma,
// webflow). Also exposes a `leases` interception command.
// Empty/unset → leases section is omitted (no HTTP call, no warning).
//
// Wired with Blockcast/onprem-k8s#357 (POST /lease/claim endpoint) and
// Blockcast/paperclip#158 (figma-bot claims on launch).
const CCROTATE_AUTH_BOT_URL = process.env.CCROTATE_AUTH_BOT_URL || '';

async function fetchLeases() {
  if (!CCROTATE_AUTH_BOT_URL) return null;
  const url = CCROTATE_AUTH_BOT_URL.replace(/\/$/, '') + '/leases';
  try {
...
function renderLeasesSection(leases) {
  if (!leases) return '';
  if (!leases.length) {
    return '🔒 Active identity leases: (none)';
  }
  const now = Date.now();
  const lines = [`🔒 Active identity leases (${leases.length}):`];
  for (const l of leases) {
    const expMs = l.expiresAt ? new Date(l.expiresAt).getTime() : 0;
    const remainMin = Math.max(0, Math.round((expMs - now) / 60000));
    const id = String(l.identity || '?').padEnd(30);
    const agent = String(l.agent || '?').padEnd(22);
    const purpose = String(l.purpose || '').padEnd(10);
    const port = l.port ? `port=${l.port}` : 'claim-only';
    lines.push(`  ${id} ${agent} ${purpose} expires in ${remainMin}m  ${port}`);
  }
  return lines.join('\n');
}
...
async function hydrateState(store, dir) {
  ...
  const files = [
    ...
    ...(typeof store.getRateLimitState === 'function'
      ? [['rate-limit-state.json', () => store.getRateLimitState()]]
      : []),
  ];
  const errors = [];
...
const HANDLERS = {
  async when({ ccrotate, store }) {
    ...
    const [table, leases] = await Promise.all([
      Promise.resolve(renderBothPools(ccrotate, Date.now())),
      fetchLeases(),
    ]);
    const sections = errs.length
      ? [`(state hydration warnings: ${errs.join('; ')})`, table]
      : [table];
    const leasesSection = renderLeasesSection(leases);
    if (leasesSection) sections.push(leasesSection);
    return sections.join('\n\n');
  },
  ...
  async leases() {
    // Standalone view: just the active identity leases held on the
    // ccrotate-auth-bot lease store. Useful when the pool table is too
    // noisy or when verifying that a designer-bot's claim landed.
    if (!CCROTATE_AUTH_BOT_URL) {
      return 'leases: CCROTATE_AUTH_BOT_URL not configured on ccrotate-serve (set the env var to enable).';
    }
    const leases = await fetchLeases();
    if (leases === null) {
      return `leases: failed to reach ${CCROTATE_AUTH_BOT_URL}/leases (timeout or non-OK response).`;
    }
    return renderLeasesSection(leases) || '🔒 Active identity leases: (none)';
  },
  ...
  async refresh({ ccrotate, store }) {
    ...
    // Walk the whole claude pool one probe at a time, spaced like the
    // `ccrotate refresh` CLI (commands/refresh.js INTER_PROBE_DELAY_MS). A
    // zero-gap pool-wide burst can stack Usage API retry-after cooldowns and
    // the trailing 429s used to get misread as exhaustion. probeOne writes
    // each result back through the StateStore (clear/mark exhausted).
  ...
  async setsession({ args }) {
    ...
    if (!setRes.ok) {
      ...
      // Auto-fallback: /setSession 400s with `email not in claude pool` when
      // the email isn't a pool member yet. Hit /register instead — same
      // {email,target,sessionKey} payload, but /register saves the session
      // AND chains the reloginViaSession internally, so the response shape
      // matches what we render below. Operator typed `/ccrotate:setSession
      // <new-email> <key>` expecting it to Just Work; the fallback collapses
      // the manual /register port-forward flow into the same slash command.
      const isUnknownEmail =
        setRes.status === 400 &&
        /email not in .* pool|not in pool/i.test(errBody);
      if (!isUnknownEmail) {
        return `setSession: bot /setSession returned ${setRes.status}. ${errBody.slice(0, 300)}`;
      }
      let regRes;
      const regController = new AbortController();
      const regTimer = setTimeout(() => regController.abort(), 120_000);
      try {
        regRes = await fetch(`${base}/register`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email, target: 'claude', sessionKey }),
        ...
        return `setSession: ${email} not in pool; /register fallback failed — ${e?.message ?? e}`;
      }
      clearTimeout(regTimer);
      const regJson = await regRes.json().catch(() => ({}));
      if (regRes.status === 409 && /SESSIONKEY_IDENTITY_MISMATCH/i.test(JSON.stringify(regJson))) {
        return [
          `setSession: ✗ sessionKey identity mismatch for ${email} (during /register fallback).`,
          `  Provided sessionKey actually belongs to: ${regJson.snappedEmail ?? '(see bot logs)'}`,
          `  ${email} still needs a sessionKey from a browser logged in as ${email}.`,
        ].join('\n');
      }
      if (!regRes.ok) {
        return `setSession: ${email} not in pool; /register fallback returned ${regRes.status}: ${(regJson?.error || '').slice(0, 300)}`;
      }
      return [
        `setSession: ✓ ${email} registered as new pool member + relogged in.`,
        `  ${regJson?.relogin?.snapStdout ?? '(no snap output)'}`,
  ...
  async help() {
    return [
      ...
      '  when         — render both Claude and Codex pool tables (+ leases if CCROTATE_AUTH_BOT_URL set)',
      '  accounts     — alias of when',
      '  leases       — show active identity leases held on ccrotate-auth-bot (requires CCROTATE_AUTH_BOT_URL)',
