...
describe('openai-client — single-key path (OPENAI_API_KEY)', () => {
  ...
  it('callImages POSTs to /v1/images/generations', async () => {
    process.env.OPENAI_API_KEY = 'sk-test-123';
    const spy = vi.spyOn(global, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({data:[{url:'https://example/img.png'}]}), { status: 200 })
    );
    const result = await callImages({model: 'gpt-image-1', prompt: 'a cat'});
    expect(spy.mock.calls[0][0]).toBe('https://api.openai.com/v1/images/generations');
    expect(result.status).toBe(200);
  });

  it('callImages falls through to the Codex/ChatGPT pool when no OPENAI_API_KEY is set', async () => {
    delete process.env.OPENAI_API_KEY;
    delete process.env.CCROTATE_CODEX_DIR;
    // No Platform key AND no codex-dir → dispatch surfaces its own configuration
    // error rather than a hard openai-client throw. We just want to prove the
    // function attempts the dispatch path and does not require a Platform key.
    await expect(callImages({model: 'gpt-image-1', prompt: 'a cat'}))
      .rejects.toThrow(/CCROTATE_CODEX_DIR|CCROTATE_STATE_URL|OPENAI_API_KEY/);
  });
