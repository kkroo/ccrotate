...
import { resetCodexAdmissionForTests } from './codex-admission.js';
...
describe('openai-client — codex pool path (CCROTATE_CODEX_DIR)', () => {
  ...
  it('frames a retryable 503 (not the raw revoked-auth string) when codex exec exhausts on revoked auth', async () => {
    // Two accounts, both fail codex exec with the OAuth-refresh revoke on stderr.
    fs.writeFileSync(path.join(dir, 'profiles.codex.json'), JSON.stringify({
      'a@x.com': realCodexProfile('a@x.com', 'IDTOK_A'),
      'b@x.com': realCodexProfile('b@x.com', 'IDTOK_B'),
    }));
    ...
    spawnSyncMock.mockImplementation(() => ({
      status: 1,
      stdout: '',
      stderr: 'Your access token could not be refreshed because you have since '
        + 'logged out or signed in to another account. Please sign in again.',
    }));

    const result = await callResponses({ model: 'gpt-5.5', input: 'hi' });

    // Both accounts tried, then framed — not the raw upstream string.
    expect(result.status).toBe(503);
    expect(result.poolExhausted).toBe(true);
    const body = await result.response.json();
    expect(body.error.code).toBe('codex_pool_needs_relogin');
    expect(body.error.type).toBe('service_unavailable');
    expect(body.error.message).not.toMatch(/signed in to another account/i);
    expect(body.error.message).toMatch(/needs re-login/i);
    // Names the offending accounts so the operator knows what to re-login.
    expect(body.error.message).toContain('a@x.com');
    expect(body.error.message).toContain('b@x.com');
...
describe('openai-client — codex admission throttle', () => {
  let dir;
  beforeEach(() => {
    dir = tmpCodexDir();
    resetCodexAdmissionForTests();
    delete process.env.OPENAI_API_KEY;
  });
  afterEach(() => {
    resetCodexAdmissionForTests();
  ...
  it('waits before the second upstream attempt when model-group rpm is capped', async () => {
    fs.writeFileSync(path.join(dir, 'profiles.codex.json'), JSON.stringify({
      'a@x.com': realCodexProfile('a@x.com', 'IDTOK_A'),
    }));
    fs.writeFileSync(path.join(dir, 'current.json'), JSON.stringify({ email: 'a@x.com' }));
    process.env.CCROTATE_CODEX_DIR = dir;
    vi.spyOn(global, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ id: 'resp_x', model: 'gpt-5.5', output_text: 'ok' }),
        { status: 200, headers: { 'content-type': 'application/json' } }),
    );
    const sleepSpy = vi.fn(async () => {});
    const admissionConfig = { rpm: 60, inputTpm: 0, outputTpm: 0, maxConcurrency: 0, maxWaitMs: 60_000 };

    await callResponses({ model: 'gpt-5.5', input: 'first' }, { admissionConfig, sleep: sleepSpy });
    await callResponses({ model: 'gpt-5.5', input: 'second' }, { admissionConfig, sleep: sleepSpy });

    // RPM=60 → one slot per second; the second call should sleep ~1000ms.
  ...
  it('logs admission_wait attribution when it delays a request', async () => {
    ...
    await callResponses(
      { model: 'gpt-5.5', input: 'first' },
      { admissionConfig, sleep: sleepSpy, attribution: { requestId: 'first', model: 'gpt-5.5' } },
    );
    await callResponses(
      { model: 'gpt-5.5', input: 'second' },
      { admissionConfig, sleep: sleepSpy, attribution: { requestId: 'second', model: 'gpt-5.5' } },
    );

    const line = logSpy.mock.calls
      .map(([msg]) => String(msg))
      .find((msg) => msg.startsWith('[openaiClient.attribution] ') && msg.includes('"event":"admission_wait"'));
    expect(line).toContain('"modelGroup":"codex-gpt-5"');
    expect(line).toContain('"rpm":60');
  });

  it('routes admission per model family — gpt-5 and gpt-4 do not share a bucket', async () => {
    fs.writeFileSync(path.join(dir, 'profiles.codex.json'), JSON.stringify({
      'a@x.com': realCodexProfile('a@x.com', 'IDTOK_A'),
    }));
    fs.writeFileSync(path.join(dir, 'current.json'), JSON.stringify({ email: 'a@x.com' }));
    process.env.CCROTATE_CODEX_DIR = dir;
    vi.spyOn(global, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ id: 'resp_x', output_text: 'ok' }),
        { status: 200, headers: { 'content-type': 'application/json' } }),
    );
    const sleepSpy = vi.fn(async () => {});
    const admissionConfig = { rpm: 60, inputTpm: 0, outputTpm: 0, maxConcurrency: 0, maxWaitMs: 60_000 };
    ...
    await callResponses({ model: 'gpt-5.5', input: 'first' }, { admissionConfig, sleep: sleepSpy });
    await callResponses({ model: 'gpt-4o', input: 'second' }, { admissionConfig, sleep: sleepSpy });
