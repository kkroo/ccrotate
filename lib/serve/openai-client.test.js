...
describe('openai-client — codex pool path (CCROTATE_CODEX_DIR)', () => {
  ...
  it('routes no-tool /v1/responses via codex exec even when OPENAI_API_KEY is also present', async () => {
    fs.writeFileSync(path.join(dir, 'profiles.codex.json'), JSON.stringify({
      'a@x.com': codexProfile('a@x.com', 'IDTOK_A'),
    }));
    fs.writeFileSync(path.join(dir, 'current.json'), JSON.stringify({ email: 'a@x.com' }));
  ...
  it('rejects tool-bearing /v1/responses even when OPENAI_API_KEY is present', async () => {
    fs.writeFileSync(path.join(dir, 'profiles.codex.json'), JSON.stringify({
      'a@x.com': codexProfile('a@x.com', 'IDTOK_A'),
    }));
    fs.writeFileSync(path.join(dir, 'current.json'), JSON.stringify({ email: 'a@x.com' }));
    ...
    const body = await result.response.json();
    expect(body.error.code).toBe('local_tools_unsupported');
  });

  it('rejects tool-bearing /v1/responses instead of executing tools in serve', async () => {
    fs.writeFileSync(path.join(dir, 'profiles.codex.json'), JSON.stringify({
      'a@x.com': codexProfile('a@x.com', 'IDTOK_A'),
    }));
    fs.writeFileSync(path.join(dir, 'current.json'), JSON.stringify({ email: 'a@x.com' }));
