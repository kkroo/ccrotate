...
describe('readLiveClaudeAuthEmail — /api/oauth/profile probe', () => {
  ...
  it('accepts both top-level and nested /api/oauth/profile email shapes', () => {
    const cc = new CCRotate();
    expect(cc._extractOauthProfileEmail({ email: 'top@x.com' })).toBe('top@x.com');
    expect(cc._extractOauthProfileEmail({ email_address: 'top-addr@x.com' })).toBe('top-addr@x.com');
    expect(cc._extractOauthProfileEmail({ account: { email: 'nested@x.com' } })).toBe('nested@x.com');
    expect(cc._extractOauthProfileEmail({ account: { email_address: 'nested-addr@x.com' } })).toBe('nested-addr@x.com');
  });
