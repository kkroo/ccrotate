...
function buildClaudeRow(email, profile, cachedAccount, now) {
  ...
  const seatMatch = seatTier.match(/^team_tier_(\d+)$/);
  let subSuffix = '';
  if (seatMatch) {
    // Team seat: label by tier only.
    subSuffix = `·t${seatMatch[1]}`;
  } else {
    // Personal account: show the rateLimitTier multiplier if present.
