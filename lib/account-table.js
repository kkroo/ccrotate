...
import { summarizeAnthropicRateLimit } from './serve/rate-limit-state.js';
...
function buildClaudeRow(email, profile, cachedAccount, now, rateLimitState) {
  ...
  const apiLimit = summarizeRelevantApiLimits(modelGroups, new Date(now));
  ...
  return {
    ...
    // The %s came from fetchAccountUsage's last-known-good cache (Usage API on
    // 429 cooldown), so they may lag a rolled window — see createTierCacheEntry.
    usageStale: !!cachedAccount?.usageStale && hasPerAccountData,
    nextReset,
    isUsableNow,
...
function shortModelGroup(group) {
  if (!group) return '?';
  const m = String(group).match(/claude-(haiku|sonnet|opus|other)/i);
  return m ? m[1].toLowerCase() : String(group);
}

function isActiveCooldown(entry, nowMs) {
...
function summarizeRelevantApiLimits(modelGroups, nowDate) {
  const entries = Object.entries(modelGroups || {}).filter(([, entry]) => entry);
  if (entries.length === 0) return { text: 'api unknown', limited: false };
  ...
  const chosen = cooldowns[0] ?? entries.sort(([a], [b]) => rank(a) - rank(b))[0];
  const [group, entry] = chosen;
  return {
    text: `${shortModelGroup(group)} ${summarizeAnthropicRateLimit(entry, nowDate)}`,
...
function buildCodexRow(email, profile, cachedAccount, now) {
  ...
  const reset5h = resetToMs(rateLimits.reset5h);
  const reset7d = resetToMs(rateLimits.reset7d);
  // Cache divergence between auth-bot (canonical) and ccrotate-serve
  // pods can leave `serviceTier: 'available'` stale on the local cache
  // even after `markCodexExhausted` writes an `exhausted` map. The
  // Claude row defends against this by reading `isAccountExhausted`;
  // mirror that here so the codex row stops mislabeling stranded
  // accounts as 🟢 / "usable now" (live bug 2026-05-27:
  // omar.ramadan93@blockcast.net and bot1@blockcast.net both showed
  // up as available while every callsite was guaranteed to get 429).
  const isExhausted = isAccountExhausted(cachedAccount, { now });
  const cachedTier = cachedAccount?.serviceTier || '?';
  const tier = isExhausted ? 'exhausted' : cachedTier;
  const exhaustedModels = Object.keys(readExhaustion(cachedAccount)).filter(m => m !== '*');
...
function renderRow(row, opts) {
  ...
  } else if (row.apiLimited) {
    availMark = '🤌';
    when = chalk.yellow(String(row.apiLimit || 'api cooldown').replace(/^([^ ]+) /, '$1 '));
  ...
  // `~` prefix + dim color flag the %s as a cached last-known-good snapshot
  // (Usage API on cooldown) rather than a live read, so a frozen "5h:100%"
  // doesn't masquerade as current when the real window has already rolled.
  const stalePrefix = row.usageStale ? '~' : '';
  const usage5h = row.usage5h != null ? `${stalePrefix}5h:${row.usage5h}%` : '';
  const usage7d = row.usage7d != null ? `${stalePrefix}7d:${row.usage7d}%` : '';
  const usagePlain = [usage5h, usage7d].filter(Boolean).join(' ').padEnd(14);
  const usage = row.usageStale ? chalk.dim(usagePlain) : usagePlain;
  const apiLimit = chalk.gray(`api: ${String(row.apiLimit || 'unknown')}`);
  ...
  if (mode === 'rich') {
    ...
    return `${indexCol} ${mark} ${tokenMark} ${availMark} ${email} ${tier} ${usage} ${when}  ${apiLimit}  ${chalk.gray(expiry)}`;
  }
  // 'when' mode: keep the back-compat shape (no #, no expires)
  return `${mark} ${tokenMark} ${availMark} ${email} ${tier} ${usage} ${when}  ${apiLimit}`;
}
...
export function renderAccountTable(ccrotate, { mode = 'rich' } = {}) {
  ...
  try {
    rateLimitState = JSON.parse(fs.readFileSync(path.join(ccrotate.profilesDir, 'rate-limit-state.json'), 'utf8'));
  } catch {
    rateLimitState = null;
  }
  const cachedAccounts = tierCacheAccountMap(cache);
  ...
  const rows = emails.map(email =>
    ccrotate.isCodexTarget()
      ...
      : buildClaudeRow(email, profiles[email], cachedAccounts.get(email), now, rateLimitState)
  );
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const mark =
      row.email === currentEmail && !row.isStale && row.tier !== 'exhausted' && !row.apiLimited
        ? '★'
        : ' ';
