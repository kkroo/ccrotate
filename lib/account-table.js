...
function buildClaudeRow(email, profile, cachedAccount, now) {
  ...
  // account are still usable. Surface which model so a one-model cap
  // doesn't read as a wholly dead account.
  const exhaustedModel =
    cachedAccount?.exhaustedModel ?? cachedAccount?.rateLimits?.exhaustedModel ?? null;
...
function renderRow(row, opts) {
  ...
  if (row.isStale) {
    ...
    availMark = '⏳';
    // Model-scoped: name the capped model so a one-model cap doesn't read
    // as a fully dead account (other models on it are still usable).
    const model = shortModelName(row.exhaustedModel);
