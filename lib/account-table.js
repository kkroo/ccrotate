...
function buildClaudeRow(email, profile, cachedAccount, now, rateLimitState) {
  ...
  const apiLimitEntry =
    modelGroups['claude-sonnet'] ??
    Object.values(modelGroups).find((entry) => entry?.cooldownUntil && Date.parse(entry.cooldownUntil) > now) ??
    Object.values(modelGroups)[0] ??
    null;
  ...
  return {
    ...
    apiLimit: summarizeAnthropicRateLimit(apiLimitEntry, new Date(now)),
  };
}
