...
// `ccrotate refresh` fired ~13 testAccount(email) probes back-to-back. Anthropic's
// per-org Usage API throttle 429s the back of the burst, and the resulting
// rate_limit_error responses were misclassified as account exhaustion in the
// tier-cache (false-flag cascade → heartbeat tier-gate deadlock).
//
...
// Sequentializing the burst keeps the rate well below Anthropic's per-org
// throttle. RefreshView's for-loop is already sequential — wrapping the
// per-account fn (rather than the loop driver) is the minimal change.
