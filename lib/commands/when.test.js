...
describe('WhenCommand', () => {
  ...
  it('does not show near-limit Codex accounts as usable now', async () => {
    const command = new WhenCommand({
      isCodexTarget: () => true,
      loadTierCache: () => ({
        updatedAt: '2025-12-31T23:30:00Z',
      ...
      loadProfiles: () => ({
        'near@example.com': { auth: { tokens: {} } }
      }),
      getCurrentAccount: () => ({ email: 'near@example.com' })
    });

    await command.execute();

    const output = logSpy.mock.calls.map(([line = '']) => line);
    expect(output.some(line => line.includes('near@example.com') && line.includes('usable now'))).toBe(false);
    expect(output.some(line => line.includes('near@example.com') && line.includes('in 3h0m'))).toBe(true);
  });
