...
import { optimizeProfile, restoreProfile, computeCrc, decodeImportPayload } from './export.js';
...
describe('decodeImportPayload', () => {
  const claudeProfile = {
    ...
    credentials: {
      claudeAiOauth: {
        accessToken: 'a', refreshToken: 'r', expiresAt: 1770000000000,
        scopes: ['chat'], subscriptionType: 'pro',
      },
    },
    oauthAccount: {
      accountUuid: 'u', emailAddress: 'claude@example.com', organizationUuid: 'o',
  ...
  it('decodes a valid blob into profiles + tier-cache', () => {
    const tierCache = { updatedAt: '2026-04-28T02:00:00.000Z', accounts: [{ email: 'claude@example.com', serviceTier: 'available' }] };
    const blob = encodeImportPayload({ 'claude@example.com': claudeProfile }, tierCache);
    const { profiles, tierCache: decodedTier } = decodeImportPayload(blob);
    expect(profiles['claude@example.com']).toEqual(claudeProfile);
    expect(decodedTier).toEqual(tierCache);
  });

  it('returns tierCache=null when the export carried none', () => {
    const blob = encodeImportPayload({ 'claude@example.com': claudeProfile });
    expect(decodeImportPayload(blob).tierCache).toBeNull();
  });

  it('strips surrounding shell quotes before decoding', () => {
    const blob = encodeImportPayload({ 'claude@example.com': claudeProfile });
    const { profiles } = decodeImportPayload(`"${blob}"`);
    expect(Object.keys(profiles)).toEqual(['claude@example.com']);
  });

  it('rejects a payload without the mp-gz-b64: prefix', () => {
    expect(() => decodeImportPayload('not-a-blob')).toThrow(/mp-gz-b64/);
  });

  it('rejects a payload with a missing CRC', () => {
    expect(() => decodeImportPayload('mp-gz-b64:AAAA')).toThrow(/CRC/);
  });

  it('rejects a corrupted payload (CRC mismatch)', () => {
    const blob = encodeImportPayload({ 'claude@example.com': claudeProfile });
    const [prefix, , data] = blob.split(':');
    expect(() => decodeImportPayload(`${prefix}:deadbeef:${data}`)).toThrow(/CRC verification failed/);
  });
});
