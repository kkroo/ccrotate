...
export class WhenCommand {
  ...
  async execute() {
    ...
    const rows = cache.accounts.map(a => {
      ...
      // Usable now if: base tier with room (5h < 95% and 7d < 95%)
      // Extra tier means user has some extra usage left — also usable unless maxed
      const isUsableNow =
        tier !== 'exhausted' &&
        (u5h == null || u5h < 95) &&
        (u7d == null || u7d < 95);
      ...
      return { email: a.email, tier, u5h, u7d, nextReset, isUsableNow, hasToken };
    });
