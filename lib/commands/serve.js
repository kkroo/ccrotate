...
import { createCostReporter } from '../serve/cost-report.js';
...
export class ServeCommand {
  ...
  async execute(options = {}) {
    ...
    // Operator-usage cost reporting (env-gated; no-op unless configured).
    // Reports ONLY traffic lacking an x-paperclip-agent-id header (human/devbox
    // CLI), so cluster-agent usage — already billed by paperclip — isn't
    // double-counted.
    const costReporter = createCostReporter({ log: { info: () => {}, warn: () => {} } });
    ...
    return new Promise((resolve, reject) => {
      ...
      server.listen(port, bind, () => {
        ...
        if (costReporter.cfg.enabled) {
          costReporter.start();
          console.log(chalk.dim(`operator cost-reporting -> ${costReporter.cfg.url} (flush ${costReporter.cfg.flushMs}ms)`));
        }
      });
      // Graceful shutdown
      const shutdown = () => {
        freshnessHandle.stop();
        // Best-effort final flush of any buffered operator usage.
        Promise.resolve(costReporter.stop()).catch(() => {}).finally(() => {
