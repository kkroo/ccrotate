  ...
  async execute(options = {}) {
    ...
    if (this.ccrotate.isCodexTarget()) {
      ...
      if (quiet) {
        ...
        const serviceTier = result.serviceTier || 'unknown';
        const tierColor = serviceTier === 'exhausted'
          ? 'red'
          : serviceTier === 'available'
            ? 'green'
            : 'yellow';
        const prefix = serviceTier === 'exhausted'
          ? '❌'
          : serviceTier === 'available'
            ? '✅'
            : '⚠️ ';
        const response = result.response || 'usage unknown (no rate-limit data)';
        console.log(chalk[tierColor](`${prefix} ${currentEmail}: ${response}`));
      }
