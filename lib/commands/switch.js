...
export class SwitchCommand {
  ...
  async execute(email, options = {}) {
    ...
    try {
      ...
      if (current.email === email) {
        ...
        if (this.ccrotate.isCodexTarget()) {
          this.ccrotate.upsertCodexProfile(email, nextProfile);
        } else {
    ...
    if (this.ccrotate.isCodexTarget()) {
      this.ccrotate.markCodexProfileStale(email, staleAt);
    } else {
      this.ccrotate.saveProfiles(profiles);
    }
  ...
  async trySwitchClaude(email, profiles, options = {}) {
    ...
    if (status === 'rate_limited') {
      console.log(chalk.yellow(`  ⚠ ${email} got 429 while verifying. Switching anyway.`));
    }
