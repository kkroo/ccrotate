...
export class SwitchCommand {
  ...
  async trySwitch(email, profiles) {
    ...
    if (status === 'rate_limited') {
      // 429 from /v1/messages is org-level, not per-account. Switch anyway —
      // the user explicitly asked, and the target account may have headroom
      // once the org burst window passes.
