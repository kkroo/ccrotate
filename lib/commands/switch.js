...
export class SwitchCommand {
  ...
  async execute(email) {
    ...
    try {
      ...
      if (current.email === email) {
        // Update profile from Keychain (source of truth) without touching Keychain
  ...
  async trySwitch(email, profiles) {
    ...
    // Validate token works before committing — prevents 401 from bad tokens
    const validated = await this.ccrotate.validateToken(credentials.claudeAiOauth.accessToken);
    if (!validated) {
      console.log(chalk.red(`  ✗ Token for ${email} rejected by API. Skipping switch.`));
      console.log(chalk.gray(`    Run /login as ${email} then \`ccrotate snap --force\` to fix.`));
      return false;
    }
