...
function buildClaudeCcrotate(opts = {}) {
  ...
  return {
    ...
    saveProfiles: vi.fn(),
    clearAnthropicRateLimitState: vi.fn(),
...
describe('SnapCommand', () => {
  ...
  describe('--email pin (claude)', () => {
    it('saves under pinned email when live identity matches the pin', async () => {
      ...
      expect(ccrotate.clearAnthropicRateLimitState).toHaveBeenCalledWith('pinned@example.com');
    });

    it('does not fail a successful snap when rate-limit cleanup fails', async () => {
      const ccrotate = buildClaudeCcrotate({
        ambientEmail: 'pinned@example.com',
        liveEmail: 'pinned@example.com',
      });
      ccrotate.clearAnthropicRateLimitState.mockImplementation(() => {
  ...
  describe('--email pin (codex)', () => {
    it('saves under the codex account when --email matches the JWT-derived identity', async () => {
      ...
      expect(ccrotate.saveProfiles).not.toHaveBeenCalled();
      expect(ccrotate.upsertCodexProfile).toHaveBeenCalledOnce();
      expect(ccrotate.upsertCodexProfile.mock.calls[0][0]).toBe('codex@example.com');
    });
