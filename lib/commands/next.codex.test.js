...
describe('Codex next rotation', () => {
  ...
  it('touches only the selected Codex profile when switching', async () => {
    const profiles = {
      'available@example.com': { auth: { tokens: {} } },
      'fresh-repaired@example.com': { auth: { tokens: {} }, stale: false },
    };
    const ccrotate = {
      isCodexTarget: () => true,
      writeActiveAccountFiles: vi.fn(),
      touchCodexProfile: vi.fn(),
      saveProfiles: vi.fn(),
    };
    const command = new NextCommand(ccrotate);

    await command.switchTo('available@example.com', profiles);

    expect(ccrotate.writeActiveAccountFiles).toHaveBeenCalledWith(profiles['available@example.com']);
    expect(ccrotate.touchCodexProfile).toHaveBeenCalledTimes(1);
    expect(ccrotate.touchCodexProfile.mock.calls[0][0]).toBe('available@example.com');
    expect(ccrotate.touchCodexProfile.mock.calls[0][1].lastUsed).toBe(profiles['available@example.com'].lastUsed);
    expect(ccrotate.saveProfiles).not.toHaveBeenCalled();
    expect(profiles['fresh-repaired@example.com'].stale).toBe(false);
  });
});
