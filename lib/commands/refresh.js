    ...
    return new Promise((resolve) => {
      ...
      const testAccount = async (email) => {
        try {
          ...
          // Try per-account /api/oauth/usage only once (1 call/hr org limit)
          const tryUsageApi = !usageApiTried;
          const testResult = await this.ccrotate.testAccount(email, { tryUsageApi });
          if (tryUsageApi) usageApiTried = true;
