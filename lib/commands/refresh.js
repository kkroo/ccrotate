...
// 13-probe burst can still trigger Usage API 429 retry-after responses and
// the trailing probes then go into per-token cooldown. Pre-T1 the classifier treated
...
// Spreading the walk over ~30s (13 × 2s) keeps refresh polite while remaining
// cheap relative to the cron's existing cadence (every 2h).
//
...
export class RefreshCommand {
  ...
  async execute() {
    ...
    // Clear ONLY expired cooldowns. Wiping active cooldowns means we ignore
    // retry-after, instantly re-429 those tokens on the next refresh cycle,
    // and never recover ("9 of 10 unknown" pattern). Honor the retry-after
    // window the API gave us last time and only let an account back into the
    // probe loop once its cooldown has genuinely expired.
    ...
    return new Promise((resolve) => {
      ...
      // Spread the per-account probes ~2s apart so a pool-wide refresh does
      // not stack retry-after cooldowns across many tokens.
  ...
  async executeCodex() {
    ...
    return new Promise((resolve) => {
      ...
      // Spread codex probes too; the usage endpoints are not a free burst API.
      const spacedTestAccount = withInterProbeDelay(testAccount);
