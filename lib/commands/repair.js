...
export class RepairCommand {
  ...
  async execute() {
    ...
    for (const email of emails) {
      ...
      // But first validate against /v1/messages (not just refresh endpoint,
      // since they have different validation)
      if (!isExpired && !isMarkedStale) {
        const valid = await this.ccrotate.validateToken(profile.credentials?.claudeAiOauth?.accessToken);
        if (valid) {
          const mins = Math.round((expiresAt - now) / 60000);
          console.log(chalk.green(`✓ healthy (${mins}min left)`));
          skipped++;
          continue;
        }
        // Token rejected by /v1/messages — fall through to refresh
        process.stdout.write(chalk.yellow('server rejected, refreshing... '));
      }
