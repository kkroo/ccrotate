...
export class NextCommand {
  ...
  async execute(options = {}) {
    ...
    const cacheStale = cacheAgeMs > 30 * 60 * 1000; // >30min = stale
    ...
    if (cache && !cacheStale) {
      ...
      for (const candidate of candidates) {
        ...
        if (cached.serviceTier === 'base') {
          console.log(chalk.green(`  ${candidate.email}: ✅ base (cached)`));
          standardAccounts.push({ ...candidate, tier: 'base' });
          break;
        } else if (cached.serviceTier === 'extra') {
          console.log(chalk.yellow(`  ${candidate.email}: ⚠️  extra (cached)`));
          extraUsageAccounts.push({ ...candidate, tier: 'extra' });
        } else {
          console.log(chalk.red(`  ${candidate.email}: ❌ ${cached.serviceTier || 'exhausted'} (cached)`));
    ...
    // Step 2: If no base found in cache, do live probes (usage API only — no /v1/messages)
    if (standardAccounts.length === 0 && (cacheStale || candidates.length > (extraUsageAccounts.length + rateLimitedAccounts.length))) {
      console.log(chalk.blue('🔍 Probing accounts (usage API only, no tokens consumed)...\n'));
      ...
      for (const candidate of toTest) {
        ...
        const result = await this.ccrotate.testAccount(candidate.email, { token, usageApiOnly: true });

        if (result.status === 'unknown') {
          // Usage API unavailable — keep whatever cache had, don't mark as rate-limited
          console.log(chalk.gray(`⏳ usage API unavailable (keeping cached tier)`));
          continue;
        }
    ...
    // Save tier cache — only overwrite accounts with real data (not "unknown")
    const allTested = [
    ...
    if (allTested.length > 0) {
      // Merge with existing cache — don't overwrite accounts we couldn't probe
      const existingCache = this.ccrotate.loadTierCache();
      const existingAccounts = existingCache?.accounts || [];
      const mergedEmails = new Set(allTested.map(a => a.email));
      const preserved = existingAccounts.filter(a => !mergedEmails.has(a.email));
      this.ccrotate.saveTierCache([...allTested, ...preserved]);
    }
    ...
    if (options.wait && resets.length > 0) {
      ...
      this.switchTo(earliest.email, profiles);
