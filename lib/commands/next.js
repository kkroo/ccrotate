...
export class NextCommand {
  ...
  async execute(options = {}) {
    // Save the current account first so `next` never rotates away from
    // a session that hasn't been captured yet.
    try {
    ...
    if (standardAccounts.length === 0 && (cacheStale || candidates.length > (extraUsageAccounts.length + rateLimitedAccounts.length))) {
      ...
      for (const candidate of toTest) {
        ...
        if (!accountData?.credentials?.claudeAiOauth) {
          console.log(chalk.gray(`⏭️  no usable credentials (skip)`));
          continue;
        }
  ...
  async executeCodex(options = {}) {
    ...
    for (const candidate of candidates) {
      ...
      } else if (result.serviceTier === 'available') {
        console.log(chalk.green(`✅ ${result.response}`));
      } else {
        console.log(chalk.yellow(`⚠️  unknown${result.response ? ` (${result.response})` : ' (no rate-limit data)'}`));
      }
    }
    ...
    const available = results
      .filter(result => result.status === 'success' && this.ccrotate.isCodexServiceTierSwitchable(result.serviceTier))
      .sort((a, b) => {
  ...
  /** Switch to an account, auto-refreshing expired or stale tokens. Returns true on success. */
  async switchTo(email, profiles) {
    ...
    const isMarkedStale = !!profiles[email]?.stale;
    const expiresAt = credentials?.claudeAiOauth?.expiresAt || 0;
    const needsRefresh = isMarkedStale || expiresAt < Date.now() + 5 * 60 * 1000;
    if (needsRefresh) {
      process.stdout.write(chalk.gray(isMarkedStale ? '  Refreshing stale token... ' : '  Refreshing expired token... '));
      ...
      if (refreshed) {
        ...
        // Symmetric with `switch.js:53-56`: when the OAuth refresh fails,
        // mark the profile `stale: true` so the auth-bot stale-poller
        // (ccrotate-auth-bot) picks it up and runs reloginViaSession from
        // the persisted /data/sessions/<hash>.json. Without this, every
        // future `next` call considers this account viable, picks it
        // again, fails refresh again — and the auth-bot never knows to
        // relogin, so the account stays in rotation indefinitely producing
        // 401s from Anthropic. Set the flag, persist, then return false
        // so the rotation tries a different candidate.
        profiles[email].stale = true;
        profiles[email].staleAt = new Date().toISOString();
        try {
          this.ccrotate.saveProfiles(profiles);
        } catch (err) {
          // Saving is best-effort: if it fails (lockfile contention,
          // etc.) the next `switch` call will still set the flag.
          console.log(chalk.gray(`  (could not persist stale flag for ${email}: ${err?.message || err})`));
        }
        console.log(chalk.yellow(`  ⚠ Token refresh failed for ${email} (stale refresh token); marked stale for auth-bot relogin`));
        return false;
      }
      console.log(chalk.green('✓'));
    }
