...
export class NextCommand {
  ...
  async execute(options = {}) {
    ...
    // Filter stale accounts out of candidacy: `stale=true` means the
    // refresh_token has been rotated elsewhere and refresh will fail with
    // invalid_grant. switchTo() refuses these (returns false) — but the
    // probe path still writes their credentials to ~/.claude/.credentials.json
    // mid-cycle, and the "earliest-reset" fallback at the end can pick one
    // and leave the active pointer corrupted. Real incident 2026-05-08:
    // active flipped to `ramadan@blockcast.net` (stale) under retry storm,
    // every subsequent run reported "Not logged in".
    // A stale account must be re-authed (snap or relogin bot) before it
    // becomes a rotation candidate again.
    const currentIndex = currentEmail ? emails.indexOf(currentEmail) : -1;
    const candidates = emails
      .map((email, i) => ({ email, index: i }))
      .filter(c => c.email !== currentEmail)
      .filter(c => !profiles[c.email]?.stale);
  ...
  async switchTo(email, profiles) {
    if (this.ccrotate.isCodexTarget()) {
      ...
      profiles[email].lastUsed = lastUsed;
      this.ccrotate.touchCodexProfile(email, { lastUsed });
      return true;
    }
