...
export class NextCommand {
  ...
  async execute(options = {}) {
    ...
    // CLI flag > config > default (prompt)
    ...
    for (const candidate of candidates) {
      ...
      standardAccounts.push({ ...candidate, tier: 'standard' });
    ...
    const allTested = [
      ...standardAccounts.map(a => ({ email: a.email, status: 'success', serviceTier: 'standard', result: '' })),
