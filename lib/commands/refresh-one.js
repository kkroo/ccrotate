  ...
  async executeClaude() {
    ...
    const scored = emails.map(email => {
      ...
      if (cached) {
        ...
        const resetAt = rl.resetAt ? new Date(rl.resetAt).getTime() : 0;
        const reset5h = rl.reset5h ? rl.reset5h * 1000 : 0;
    ...
    const available = scored.filter(s => !s.onCooldown && !s.skip);
  ...
  checkPreemptiveSwitch(emails, profiles) {
    ...
    const candidates = cache.accounts
      ...
      .map(a => {
        ...
        const resetAt = arl.resetAt ? new Date(arl.resetAt).getTime() : null;
        const reset5h = arl.reset5h ? arl.reset5h * 1000 : null;

        // 5h reset already passed → account effectively reset
        const reset5hPassed = reset5h && reset5h < nowMs;
        // Per-account API: resetAt is 7d reset. We don't have separate 5h.
        // Estimate: if account showed high 5h and data is > 5hrs old, assume reset
        ...
        else if (a.serviceTier === 'exhausted') score = 50;
  ...
  async executeCodex() {
    ...
    const scored = emails.map(email => {
      ...
      if (cached) {
        const rl = cached.rateLimits || {};
        const resetAt = rl.resetAt ? new Date(rl.resetAt).getTime() : 0;
        const reset5h = rl.reset5h ? new Date(rl.reset5h).getTime() : 0;
        const nextReset = Math.max(resetAt, reset5h);
    ...
    scored.sort((a, b) => a.score - b.score || a.email.localeCompare(b.email));
    const available = scored.filter(s => !s.skip && s.hasAuth);
