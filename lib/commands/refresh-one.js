...
export class RefreshOneCommand {
  ...
  async executeClaude(requestedEmail) {
    ...
    const scored = emails.map(email => {
      ...
      if (profiles[email]?.stale) return { email, score: 999, reason: 'stale profile', onCooldown, skip: true };
    ...
    // A specifically requested account is an operator action, not a cron tick:
    // let it bypass the local cooldown file once and make a real Usage API
    // attempt. If Anthropic still returns 429/unavailable, testAccount reports
    // unknown below and the caller gets the real failure mode.
    scored.sort((a, b) => a.score - b.score || a.email.localeCompare(b.email));
    const available = scored.filter(s => (!s.onCooldown || s.requested) && (!s.skip || s.requested));
    ...
    if (requestedProfileEmail && !available.some(s => s.email === requestedProfileEmail)) {
      ...
      if (target?.skip) {
        throw new Error(`${requestedProfileEmail} is not refreshable (${target.reason})`);
      }
    ...
    if (available.length === 0) {
      ...
      const nonSkipped = scored.filter(s => !s.skip);
      if (nonSkipped.length > 0 && nonSkipped.every(s => s.onCooldown)) {
        ...
        console.log(chalk.yellow('No refreshable Claude accounts available right now.'));
      }
      return;
    }
    ...
    const result = await this.ccrotate.testAccount(target.email, {
      token,
      usageApiOnly: true,
      ignoreUsageApiCooldown: !!requestedProfileEmail,
    });

    if (result.status === 'unknown') {
      if (requestedProfileEmail) {
        throw new Error(`${target.email} Usage API unavailable; Anthropic may still be enforcing a cooldown. Retry later, or paste a fresh sessionKey if the profile remains stale.`);
      }
