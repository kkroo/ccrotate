...
// `email` is the keyed identity from the caller's profiles map. It's
// accepted positionally for symmetry with restoreProfile and because all
// known call sites already have it on hand — keeping the signature stable
// avoids churning every call site every time the compact shape changes.
// Currently unused inside, hence the underscore prefix.
export function optimizeProfile(_email, profile) {
