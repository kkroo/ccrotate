...
describe('saveProfiles — protected by active-files lock', () => {
  it('holds the active-files lock while staging+renaming', () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ccrotate-saveprofiles-lock-'));
    const ccrotate = new CCRotate();
    ccrotate.profilesDir = tempRoot;
    ccrotate.profilesFile = path.join(tempRoot, 'profiles.json');
    const lockPath = path.join(tempRoot, '.active-files.lock');

    // Spy on withActiveFilesLock so we can confirm saveProfiles routes
    // through it. The race we're closing: if saveProfiles ever wrote
    // outside the lock, two concurrent writers could each stage
    // profiles.json.tmp, one would rename first, and the second's rename
    // would ENOENT — losing that writer's update silently.
    const spy = vi.spyOn(ccrotate, 'withActiveFilesLock');
    ccrotate.saveProfiles({ 'a@example.com': { provider: 'claude' } });

    expect(spy).toHaveBeenCalledTimes(1);
    expect(fs.existsSync(lockPath)).toBe(false); // released after the write
    expect(JSON.parse(fs.readFileSync(ccrotate.profilesFile, 'utf8'))).toEqual({
      'a@example.com': { provider: 'claude' },
    });
  });

  it('serializes concurrent writers — the second waits, no ENOENT race', () => {
    // Reproduces the exact paperclip-0 lifecycle-hook stderr we saw
    // pre-fix: "ENOENT: no such file or directory, rename
    // 'profiles.json.tmp' -> 'profiles.json'". Pre-fix, two unlocked
    // writers each created profiles.json.tmp; whichever renamed first
    // won, the other's rename ENOENTed. With the lock, they serialize.
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'ccrotate-saveprofiles-race-'));
    const ccrotate = new CCRotate();
    ccrotate.profilesDir = tempRoot;
    ccrotate.profilesFile = path.join(tempRoot, 'profiles.json');
    const lockPath = path.join(tempRoot, '.active-files.lock');

    // Plant a held lock from a "concurrent writer" that hasn't released
    // yet. saveProfiles should time out rather than racing the rename.
    const fd = fs.openSync(lockPath, 'wx');
    try {
      // Use the smallest acceptable timeout to keep the test fast.
      const original = ccrotate.withActiveFilesLock.bind(ccrotate);
      vi.spyOn(ccrotate, 'withActiveFilesLock').mockImplementation((fn, opts = {}) =>
        original(fn, { ...opts, timeout: 100, staleMs: 60_000 })
      );

      expect(() =>
        ccrotate.saveProfiles({ 'a@example.com': { provider: 'claude' } })
      ).toThrow(/Failed to save profiles.*timed out waiting/);
      // No half-written profiles.json: writer never started body.
      expect(fs.existsSync(ccrotate.profilesFile)).toBe(false);
    } finally {
      fs.closeSync(fd);
