...
describe('CCRotate API sync timestamps', () => {
  ...
  describe('probeCodexAccountAsync (async sibling for serve-codex freshness-loop)', () => {
    ...
    it('does NOT call the sync runCodexProbe (uses runCodexProbeAsync)', async () => {
      const ccrotate = new CCRotate();
      const profiles = {
        'codex@example.com': {
          auth: { tokens: { id_token: 'jwt' } },
      ...
      const asyncSpy = vi.spyOn(ccrotate, 'runCodexProbeAsync').mockResolvedValue({
        status: 'success',
        snapshot: {
          capturedAt: '2026-04-28T04:00:00.000Z',
    ...
    it('returns error result without probing when accountData has no auth', async () => {
      const ccrotate = new CCRotate();
      const probeSpy = vi.spyOn(ccrotate, 'runCodexProbeAsync').mockResolvedValue({});
