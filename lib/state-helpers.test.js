...
import { withCcrotateLock, markAccountExhausted, clearAccountExhausted, applyImport, isAccountExhausted } from './state-helpers.js';
...
function claudeProfile(email, { lastApiSyncAt = null, accessToken = 'tok' } = {}) {
  return {
    provider: 'claude',
    credentials: {
      claudeAiOauth: {
        accessToken, refreshToken: 'r', expiresAt: 1770000000000,
...
function codexProfile(email) {
  return {
    provider: 'codex',
    auth: { tokens: { account_id: email, id_token: 'id' } },
    accountId: email, name: 'Codex', tokenClaims: null,
  };
}
...
describe('state-helpers', () => {
  ...
  describe('markAccountExhausted', () => {
    it('writes a fresh tier-cache entry when none existed', () => {
      ...
      // serviceTier stays account-health (null here — no prior tier);
      // exhaustion lives in the model-scoped `exhausted` map under '*'.
      expect(entry.serviceTier).toBeNull();
      expect(isAccountExhausted(entry)).toBe(true);
      expect(entry.exhausted['*'].reset5h).toBe(reset5h);
    ...
    it('does not inherit a stale "cleared" response when re-marking after a clear', () => {
      // Bug: markAccountExhausted fell back to existing?.response when no
      // response was passed. After clearAccountExhausted (which sets
      // response:'cleared by successful upstream response'), the next mark
      // re-stamped serviceTier:'exhausted' while inheriting that "cleared"
      // string — a self-contradictory row.
      const dir = tmpDir();
      const reset5h = Math.floor(Date.now() / 1000) + 3600;
      ...
      expect(isAccountExhausted(entry)).toBe(true);
      expect(entry.response).not.toContain('cleared by successful');
      expect(entry.response).toMatch(/quota exhausted/);
    });
    ...
    it('can mark runtime exhaustion for a specific model', () => {
      ...
      const cache = JSON.parse(fs.readFileSync(path.join(dir, 'tier-cache.json'), 'utf8'));
      const entry = cache.accounts[0];
      // Exhaustion is recorded under the model key, not as an account-wide
      // serviceTier — opus capped does not exhaust haiku on the same account.
      expect(entry.exhausted['claude-opus-4-7']).toBeTruthy();
      expect(isAccountExhausted(entry, { model: 'claude-opus-4-7' })).toBe(true);
      expect(isAccountExhausted(entry, { model: 'claude-haiku-4-5-20251001' })).toBe(false);
    });
    ...
    it('upserts an existing entry, preserving prior rateLimits fields and replacing only what we know', () => {
      ...
      // Account-health tier ('base') is preserved through exhaustion now —
      // exhaustion is the model-scoped map, not an overwrite of serviceTier.
      expect(a.serviceTier).toBe('base');
      expect(a.exhausted['*'].reset5h).toBe(reset5h);
    ...
    it('still marks exhausted when utilization data is stale even if percentages are low', () => {
      ...
      const cache = JSON.parse(fs.readFileSync(tierCachePath, 'utf8'));
      expect(isAccountExhausted(cache.accounts[0])).toBe(true);
    ...
    it('REFUSES to write when tier-cache exists but is unparseable (no mass-shrink)', () => {
      // Live incident 2026-05-21T01:19Z: tier-cache.json hit a transient
      // cephfs rename race that left it unparseable. The previous catch-
      // and-swallow path treated the unparseable file as "fresh cache"
      // and wrote a single-entry replacement, atomically nuking a 15-
      // account file. Now: on parse-error, abort the write so the next
      // probe / read cycle can recover the existing file. Falling back
      // to empty is only safe when the file is GENUINELY ABSENT.
      const dir = tmpDir();
      const tierCachePath = path.join(dir, 'tier-cache.json');
      // Seed a populated cache (the file the live pool would have on disk).
      const populatedCache = {
        updatedAt: new Date().toISOString(),
        accounts: [
          { email: 'a@example.com', serviceTier: 'base', rateLimits: { utilization5h: 12 } },
          { email: 'b@example.com', serviceTier: 'base', rateLimits: { utilization5h: 50 } },
          { email: 'c@example.com', serviceTier: 'base', rateLimits: { utilization5h: 80 } },
        ],
      };
      fs.writeFileSync(tierCachePath, JSON.stringify(populatedCache));
      // Corrupt the file in place (simulates the transient cephfs window).
      fs.writeFileSync(tierCachePath, '{ "accounts": [ this is not valid json');

      const result = markAccountExhausted(dir, 'd@example.com', { reset5h: Math.floor(Date.now() / 1000) + 3600 });
      expect(result).toMatchObject({ skipped: true });
      expect(result.reason).toContain('unparseable');

      // The on-disk file MUST be left exactly as the test wrote it —
      // no atomic rename replaced the populated cache with a 1-entry one.
      expect(fs.readFileSync(tierCachePath, 'utf8')).toBe('{ "accounts": [ this is not valid json');
    });

    it('writes a fresh-install entry when tier-cache file is absent (distinguishes ENOENT from corruption)', () => {
      const dir = tmpDir();
      // No tier-cache.json on disk.
      const result = markAccountExhausted(dir, 'fresh@example.com', { reset5h: Math.floor(Date.now() / 1000) + 3600 });
      expect(result).toMatchObject({ skipped: false });
      const cache = JSON.parse(fs.readFileSync(path.join(dir, 'tier-cache.json'), 'utf8'));
      expect(cache.accounts).toHaveLength(1);
      expect(cache.accounts[0].email).toBe('fresh@example.com');
    });
  });

  describe('readExhaustion / isAccountExhausted', () => {
    const future = () => Math.floor(Date.now() / 1000) + 3600;
    const past = () => Math.floor(Date.now() / 1000) - 3600;

    it('reads the model-scoped exhausted map', () => {
      const entry = {
        serviceTier: 'base',
        exhausted: { 'claude-haiku-4-5-20251001': { reset5h: future() } },
      };
      expect(isAccountExhausted(entry, { model: 'claude-haiku-4-5-20251001' })).toBe(true);
      // A haiku cap does not exhaust opus on the same account.
      expect(isAccountExhausted(entry, { model: 'claude-opus-4-7' })).toBe(false);
    });

    it('treats a "*" record as account-wide — matches every model', () => {
      const entry = { serviceTier: null, exhausted: { '*': { reset5h: future() } } };
      expect(isAccountExhausted(entry, { model: 'claude-opus-4-7' })).toBe(true);
      expect(isAccountExhausted(entry, { model: 'anything' })).toBe(true);
    });

    it('reads the legacy serviceTier:"exhausted" + exhaustedModel shape', () => {
      const legacy = {
        serviceTier: 'exhausted',
        exhaustedModel: 'claude-opus-4-7',
        rateLimits: { reset5h: future() },
      };
      expect(isAccountExhausted(legacy, { model: 'claude-opus-4-7' })).toBe(true);
      expect(isAccountExhausted(legacy, { model: 'claude-haiku-4-5-20251001' })).toBe(false);
    });

    it('does not count a record whose reset epoch has passed', () => {
      const entry = { exhausted: { '*': { reset5h: past() } } };
      expect(isAccountExhausted(entry)).toBe(false);
    });

    // Recovery fallback (2026-05-20): some tier-cache writes lose the
    // `exhausted` map but preserve the markAccountExhausted-shaped
    // `response` + `rateLimits.reset5h/7d`. Selection loop was walking
    // these accounts as if usable. Reconstruct the '*' record so they're
    // skipped until the on-disk reset epoch passes.
    it('reconstructs a "*" record from response + rateLimits.reset5h when the exhausted map is missing', () => {
      const orphan = {
        serviceTier: 'base',
        response: 'quota exhausted; resets at 2099-01-01T00:00:00.000Z',
        rateLimits: { reset5h: future() },
      };
      expect(isAccountExhausted(orphan, { model: 'claude-opus-4-7' })).toBe(true);
      expect(isAccountExhausted(orphan, { model: 'anything' })).toBe(true);
    });

    it('reconstructs from reset7d alone when reset5h is absent', () => {
      const orphan = {
        serviceTier: null,
        response: 'quota exhausted; resets at 2099-01-01T00:00:00.000Z',
        rateLimits: { reset7d: future() },
      };
      expect(isAccountExhausted(orphan)).toBe(true);
    });

    it('does not trigger the response fallback when the reset epoch has passed', () => {
      const stale = {
        serviceTier: 'base',
        response: 'quota exhausted; resets at 1970-01-01T00:00:00.000Z',
        rateLimits: { reset5h: past() },
      };
      expect(isAccountExhausted(stale)).toBe(false);
    });

    it('does not trigger the response fallback for unrelated response strings', () => {
      const benign = {
        serviceTier: 'base',
        response: 'cleared by successful upstream response',
        rateLimits: { reset5h: future() },
      };
      expect(isAccountExhausted(benign)).toBe(false);
    });

    it('does not trigger the response fallback when rateLimits has no reset epoch', () => {
      const noReset = {
        serviceTier: 'base',
        response: 'quota exhausted',
        rateLimits: { utilization5h: 100 },
      };
      expect(isAccountExhausted(noReset)).toBe(false);
    });

    it('prefers the explicit exhausted map over the response fallback', () => {
      // If both are present, the map wins — including model-scoping. The
      // fallback only fires when the map is absent. A response saying
      // "quota exhausted" alongside a model-scoped haiku-only record
      // must NOT exhaust opus.
      const withMap = {
        serviceTier: 'base',
        exhausted: { 'claude-haiku-4-5-20251001': { reset5h: future() } },
        response: 'quota exhausted; resets at 2099-01-01T00:00:00.000Z',
      ...
      expect(isAccountExhausted(withMap, { model: 'claude-haiku-4-5-20251001' })).toBe(true);
      expect(isAccountExhausted(withMap, { model: 'claude-opus-4-7' })).toBe(false);
    });
  });
  ...
  describe('clearAccountExhausted', () => {
    ...
    it('preserves the label when exhaustedModel does not match the success model', () => {
      ...
      // Untouched — changed:false means no write, the legacy entry stands.
      const entry = JSON.parse(fs.readFileSync(tierCachePath, 'utf8')).accounts[0];
    ...
    it('clears when exhaustedModel matches the success model', () => {
      ...
      expect(result).toMatchObject({ changed: true, clearedModels: ['claude-opus-4-7'] });
      const entry = JSON.parse(fs.readFileSync(tierCachePath, 'utf8')).accounts[0];
      expect(entry.serviceTier).toBeNull();
      expect(isAccountExhausted(entry)).toBe(false);
    });
    ...
    it('returns the unparseable reason and PRESERVES the on-disk file when cache is corrupt', () => {
      // Mirrors the markAccountExhausted abort: a writer encountering a
      // corrupt cache must not interpret it as "no entry" and silently
      // succeed without acting. Returning a distinct reason keeps the
      // RCA visible in logs.
      const dir = tmpDir();
      const tierCachePath = path.join(dir, 'tier-cache.json');
      fs.writeFileSync(tierCachePath, '{ corrupted');
      const result = clearAccountExhausted(dir, 'a@example.com');
      expect(result.changed).toBe(false);
      expect(result.reason).toContain('unparseable');
      expect(fs.readFileSync(tierCachePath, 'utf8')).toBe('{ corrupted');
    });
  });

  describe('applyImport', () => {
    it('adds a new claude profile to a fresh profiles.json', () => {
      const dir = tmpDir();
      const blob = encodeImportPayload({ 'a@x.com': claudeProfile('a@x.com') });
      const result = applyImport(dir, blob);
      expect(result).toMatchObject({ accounts: 1, added: 1, updated: 0, kept: 0 });
      const profiles = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
      expect(profiles['a@x.com'].provider).toBe('claude');
    });

    it('overwrites a local profile when the incoming record synced more recently', () => {
      const dir = tmpDir();
      fs.writeFileSync(path.join(dir, 'profiles.json'), JSON.stringify({
        'a@x.com': claudeProfile('a@x.com', { lastApiSyncAt: '2026-01-01T00:00:00.000Z', accessToken: 'old' }),
      }));
      const blob = encodeImportPayload({
        'a@x.com': claudeProfile('a@x.com', { lastApiSyncAt: '2026-05-01T00:00:00.000Z', accessToken: 'new' }),
      });
      const result = applyImport(dir, blob);
      expect(result).toMatchObject({ added: 0, updated: 1, kept: 0 });
      const profiles = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
      expect(profiles['a@x.com'].credentials.claudeAiOauth.accessToken).toBe('new');
    });

    it('keeps the local profile when it synced more recently than the incoming one', () => {
      const dir = tmpDir();
      fs.writeFileSync(path.join(dir, 'profiles.json'), JSON.stringify({
        'a@x.com': claudeProfile('a@x.com', { lastApiSyncAt: '2026-05-01T00:00:00.000Z', accessToken: 'local' }),
      }));
      const blob = encodeImportPayload({
        'a@x.com': claudeProfile('a@x.com', { lastApiSyncAt: '2026-01-01T00:00:00.000Z', accessToken: 'remote' }),
      });
      const result = applyImport(dir, blob);
      expect(result).toMatchObject({ added: 0, updated: 0, kept: 1 });
      const profiles = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
      expect(profiles['a@x.com'].credentials.claudeAiOauth.accessToken).toBe('local');
    });

    it('routes codex profiles to profiles.codex.json', () => {
      const dir = tmpDir();
      const blob = encodeImportPayload({
        'a@x.com': claudeProfile('a@x.com'),
        'c@x.com': codexProfile('c@x.com'),
      });
      const result = applyImport(dir, blob);
      expect(result.accounts).toBe(2);
      const claude = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.json'), 'utf8'));
      const codex = JSON.parse(fs.readFileSync(path.join(dir, 'profiles.codex.json'), 'utf8'));
      expect(Object.keys(claude)).toEqual(['a@x.com']);
      expect(Object.keys(codex)).toEqual(['c@x.com']);
    });

    it('merges the export tier-cache into tier-cache.json', () => {
      const dir = tmpDir();
      const tierCache = {
        updatedAt: '2026-05-01T00:00:00.000Z',
        accounts: [{ email: 'a@x.com', serviceTier: 'base', syncedAt: '2026-05-01T00:00:00.000Z' }],
      };
      const blob = encodeImportPayload({ 'a@x.com': claudeProfile('a@x.com') }, tierCache);
      const result = applyImport(dir, blob);
      expect(result.tierMerged).toBe(1);
      const cache = JSON.parse(fs.readFileSync(path.join(dir, 'tier-cache.json'), 'utf8'));
      expect(cache.accounts[0].email).toBe('a@x.com');
    });

    it('throws on a corrupted blob (CRC mismatch)', () => {
      const dir = tmpDir();
      const blob = encodeImportPayload({ 'a@x.com': claudeProfile('a@x.com') });
      const [prefix, , data] = blob.split(':');
      expect(() => applyImport(dir, `${prefix}:deadbeef:${data}`)).toThrow(/CRC/);
    });
  });
});
